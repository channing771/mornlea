...
| Font | No project-bundled font | Godot Engine built-in default | 4.7.2-stable | No separate project asset | 不提取或单独分发字体文件，并保留 Godot 二进制附带的声明。 | 只允许使用引擎内置默认字体；禁止下载或复制字体。 |
| Material | Mornlea generated materials | packages/client/assets | 源提交 84f3e0e75dff6987e47cf2aa50f50d888e85eafb | MIT (project-owned) | 从项目自有注册表确定性生成衍生资源并保留根 LICENSE；禁止加入 Mojang 或未登记的第三方美术资源。 | 仅在生成文件清单与校验和验证通过后允许使用。 |
