...
服务端 SHALL 在权威采掘的全部预检通过、目标方块被实际移除之后，从玩家当时选中的快捷栏工具扣减恰好一点耐久。服务端 SHALL 同样在**不移除方块的成功工具动作**之后扣减恰好一点耐久——当前唯一这样的动作是用锄头把泥土或草翻成耕地。判定标准是「工具确实完成了一次有效作用」，而不是「方块是否被移除」。该判定的唯一豁免是「作物 × 锄头」：被移除的方块是作物（小麦八个生长阶段之一）且完成时选中物是完好锄头（损坏形态不算锄头）时，服务端 SHALL NOT 扣减耐久——锄头只在翻地时磨损。持其他工具收获作物、或持锄头破坏非作物方块，仍 MUST 各扣减恰好一点耐久。
