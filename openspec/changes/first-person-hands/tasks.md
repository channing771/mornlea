...
- [ ] 7.1 斜持姿态：双手改自左下/右下屏角斜向入画（向中心倾斜 20°–35°，右手为主手），世界烘焙根变换多乘斜持旋转；以 capture 实拍 PNG 目检迭代锁定（≤5 轮，逐轮记录数值与结论），锁定后常量 + 落点测试（含斜持倾角断言）钉死；验证 `go test ./packages/client/render -race -count=1`。
