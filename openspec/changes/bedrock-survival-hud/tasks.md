...
- [ ] 3.14 implementer 自证后以追加提交生成覆盖 Task 4B `BASE..HEAD` 的官方 committed review package，包含 commit list、stat、full diff 与 raw diff SHA-256；交给全新独立 task reviewer 同时给出 SPEC/QUALITY 双裁决。Task 4B finding 只以追加 fix commit 修复并由同一 scoped reviewer 复审，独立最多 5 轮；通过前 3.10 保持未勾选。
