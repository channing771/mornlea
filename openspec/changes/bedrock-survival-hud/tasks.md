...
- [ ] 3.11 Task 4B 先在 `internal/render/hud/chat_test.go` 写真实 RED：固定 `44x40` open 的六行 history + 32 个“界”、`109x40` open 的六行 history + 32 个内嵌窄 Latin `i`，并围绕两处 fit 临界值做小型确定性整数尺寸 open/closed 扫描；逐一断言 panel/glyph finite、严格位于 framebuffer 内且不与 survival status 相交，同时保留六行、input、两个 panel、448 glyph、glyph ink containment、零分配与容量门禁。
- [ ] 3.12 在 `internal/render/hud/chat.go` 一次修复共享聊天缩放根因：优先从可用宽高扣固定亚像素余量后再求统一 scale，极小可用量钳到零；不得盲目叠加 `Nextafter32`、按 panel/glyph 裁切、删行、截短文本、改变 uniform scale、容量或 capture 分支。
- [ ] 3.13 运行 `gofmt -w internal/render/hud/chat.go internal/render/hud/chat_test.go`、chat focused、`go test ./internal/render/hud -race -count=1`、chat/capture integration focused、archcheck、benchmark scenario v18/layout focused、OpenSpec strict、`gofmt -l` 与 `git diff --check`；确认 0 alloc、448 glyph、两个 panel、合法最大 76/245 和固定 247/700/12288/45888 门禁继续通过，且 Task 5 capture/golden 无改动。
