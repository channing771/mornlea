...
| 3 hotbar/mining | `/root/hud_task3_hotbar` | `4aabc6a`（候选）/ `3e047b5`（候选后打开态修复）/ `ad2c6bb`（round 1） | `/root/hud_task3_review`：首轮 SPEC FAIL（1 个 P2）；round 1 待复审 | `/root/hud_task3_review`：首轮 QUALITY FAIL（1 个 P2）；round 1 待复审 | 1 | `make rust` EXIT 0；快捷栏与采掘红测均按预期失败；round 1 以 BASE archive 叠加 renderer 测试真实红测失败；renderer focused、HUD race、archcheck、gofmt、diff check 均 EXIT 0 | 待 controller 裁决 |
...
| 3 | 1 | `/root/hud_task3_review` | P2：`renderer_test.go` 未进入候选范围，导致 2.1 的勾选和 renderer 层 TDD 证据不真实 | `ad2c6bb` 新增 renderer 级关闭态/采掘实例前缀见证；在 BASE archive 叠加该测试后真实失败；focused、HUD race、archcheck、gofmt、diff check 均 EXIT 0 | 待 round 1 复审 | `/root`：要求最小补测与证据 |
