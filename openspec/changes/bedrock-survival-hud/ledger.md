...
| 4 status/layout/capacity | `/root/hud_task4_status` | `feat: align survival status around hotbar`（本候选提交，hash 待 controller 回填） | 待独立评审 | 待独立评审 | 0 | `make rust` EXIT 0；生命/氧气/响应式/容量均保留真实 RED，focused GREEN；HUD race、archcheck、目标文件 gofmt、diff check、`encode.go` 不变均 EXIT 0 | 待 controller 裁决 |
