...
| 4 status/layout/capacity | `/root/hud_task4_status` | `92f0c64`（候选）/ `5e7f970`（round 1 docs fix）/ `a15ac66`（round 1 code fix）/ `04ecf8c`（round 2 comment fix）/ `ade4768`（round 2 verification）/ `02a7a8a`（round 3 downstream fix）/ `fix: keep chat clear of survival status`（round 4 fix，hash 待回填） | `/root/hud_task4_review`：首轮 SPEC FAIL（P1/P2）；round 1/2/3 复审 SPEC PASS；round 4 待复审 | `/root/hud_task4_review`：首轮 QUALITY FAIL（P1/P2）；round 1 复审 QUALITY FAIL（新 P3 注释漂移）；round 2/3 复审 QUALITY PASS；round 4 待复审 | 4 | 全部生命/氧气、响应式、容量与下游集成分段均真实 RED/GREEN；round 4 以 640×360 status `y=268..284` 与 history panel `y=244..284` 重叠为 RED，共享 closed-hotbar status anchor 后 GREEN；HUD race、chat/capture scene focused、archcheck、benchmark v18 focused、OpenSpec strict 56/0、gofmt、diff check 均 PASS | `/root`：chat history/input 整体从关闭态 status top 向上锚定，不隐藏状态/不删事件/不加 capture 分支；3.10 暂不勾 |
...
| 4 | 4 | `/root/hud_task5_review` | P2：`ai-companion` 的 accepted chat history `y=244..284` 与关闭态 survival status `y=268..284` 重叠，破坏聊天语义可审查性 | `chat_test.go` 真实 RED 同帧构造 accepted line、open input、confirmed health 与 depleted oxygen，机械检查 panel/glyph 与status quad 不相交；`chat.go` 最小复用 `hotbarRowBounds(false)` 的 status top 向上锚定；0 alloc、六行+input 容量与 glyph ink 测试均 GREEN | round 4 待复审 | `/root`：修 shared chat root cause，不加 capture 分支/API，不改容量/ABI |
