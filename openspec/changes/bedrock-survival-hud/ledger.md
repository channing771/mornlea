...
| 4 status/layout/capacity | `/root/hud_task4_status` | `92f0c64`（候选）/ `5e7f970`（round 1 docs fix）/ `a15ac66`（round 1 code fix）/ `04ecf8c`（round 2 comment fix） | `/root/hud_task4_review`：首轮 SPEC FAIL（P1/P2）；round 1 复审 SPEC PASS | `/root/hud_task4_review`：首轮 QUALITY FAIL（P1/P2）；round 1 复审 QUALITY FAIL（新 P3 注释漂移）；round 2 复审待执行 | 2 | round 1 package raw SHA-256 `9a620eb3688c515b8ba8bef2f280b67a1e585ac2b3b4a455b15b20fa509d50d3`；P1/P2 复审 ADDRESSED；round 2 只修正两处容量注释；HUD race、archcheck、benchmark focused、OpenSpec strict 56/0、gofmt、diff check 均 EXIT 0 | `/root`：保持 scenario v18 固定布局；3.10 待最终双 PASS |
...
| 4 | 1 | `/root/hud_task4_review` | P1：候选把固定 HUD 布局移到 265/13056/46656，却仍标记 benchmark scenario v18，违反主规格锁定的 247/12288/45888 | `5e7f970` 先把 active change 改为 resolved-slot 与 v18 固定契约；`a15ac66` 每个生命/氧气槽只发一个实例，合法最大 76/245，固定布局恢复 247/12288/45888 | round 1 复审：ADDRESSED，SPEC PASS | `/root`：不升 v19、不新增 benchmark delta；用视觉等价 resolved-slot 保持 v18 |
| 4 | 1 | `/root/hud_task4_review` | P2：打开态分隔线 `max(scale*2,1)` 在极窄正尺寸 framebuffer 越过下沿 | `a15ac66` 删除 1px floor；新增 17×800、800×17、16×16、1×1 的打开/关闭全部 quad/glyph finite 且界内回归 | round 1 复审：ADDRESSED，SPEC PASS | `/root`：最小修复并覆盖更小正尺寸 |
| 4 | 2 | `/root/hud_task4_review` | P3：容量测试注释仍声称打开态恰好打满上限且容量可随有界实例更新，与合法最大 245、scenario v18 固定 247 矛盾 | `04ecf8c` 只把两处注释改为保留 2 quad 余量且只有显式 benchmark scenario 迁移才能改变固定布局；测试逻辑与生产代码不变 | round 1 复审：QUALITY FAIL；round 2 待复审 | `/root`：最小注释修复，3.10 不勾 |
