...
| 4 status/layout/capacity | `/root/hud_task4_status` | `92f0c64`（候选）/ `5e7f970`（round 1 docs fix）/ `a15ac66`（round 1 code fix）/ `04ecf8c`（round 2 comment fix）/ `ade4768`（round 2 verification）/ `test: align downstream survival HUD contract`（round 3 fix，hash 待回填） | `/root/hud_task4_review`：首轮 SPEC FAIL（P1/P2）；round 1/2 复审 SPEC PASS；round 3 待复审 | `/root/hud_task4_review`：首轮 QUALITY FAIL（P1/P2）；round 1 复审 QUALITY FAIL（新 P3 注释漂移）；round 2 复审 QUALITY PASS；round 3 待复审 | 3 | 既有生命/氧气、响应式与容量分段均真实 RED/GREEN；round 3 `cmd/mornlea` race 先以 health 10 vs 16、oxygen 480 vs 96 bytes RED，后以 185.424s GREEN；HUD race、archcheck、benchmark v18 focused、OpenSpec strict 56/0、gofmt、diff check 均 PASS | `/root`：选择 resolved single-slot 保持 benchmark v18 的 247/700/12288/45888，修正下游精确契约；3.10 暂不勾 |
...
| 4 | 3 | Task 5 integration | late downstream finding：`cmd/mornlea/chat_test.go` 与 `health_hud_test.go` 仍锁定 Task 4 前 health 16 quad/oxygen 2 quad，与已评审 resolved-slot 各10 quad 契约矛盾 | focused race EXIT 1：health 10 vs 16，oxygen 480 vs 96 bytes；round 3 精确更新所有同类断言与说明，不改生产代码；同一 focused 与完整 `cmd/mornlea` race 均 GREEN | round 3 待复审 | `/root`：修正测试根因，不放宽断言，保留 Task 5 未提交改动 |
