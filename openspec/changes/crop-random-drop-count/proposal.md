...
- **`internal/core/item.go:222` 的「完整产出是 1 小麦 + 2 种子」注释失真**：该文件属 A-01（权威格子工作台）独占集，本 change 不得触碰；由 A-01 合流集成或后续独立修复行清偿。
- **`internal/companion/plan_types.go:499` 的「第二份产物（2 种子）」注释失真**：不属任何认领行的冻结集，但按范围冻结不顺手扩大本 change 文件集；留待后续小型修复队列（F 组）或集成期一并清偿。
- **基线文档「掉落固定不随机：成熟作物 1 小麦 + 2 种子」句**：随本 change 归档收尾在 `AGENTS.md`/`CLAUDE.md`/`progress.md` 同步改写；若与第一批次 A-06/A-07 合流窗口冲突，按集成裁决顺延。
