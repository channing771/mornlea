...
- **BREAKING（仅畸形或不适合单行编辑的本机配置）**：`texturePackPath` 统一限制为最多 1024 个 UTF-8 字节且不得含 CR/LF；此前可被 JSON 解析但无法形成 Darwin 可用文件系统路径的超长值，或合法指向含换行文件名却无法安全进入单行设置控件的值，将改为带字段上下文的加载错误。用户迁移方式是缩短/清空该字段，或把目录改为单行名称后更新路径。
...
- 兼容性：配置 JSON 仍为 v1，普通旧文件无需迁移；唯一收紧是上述超长或含 CR/LF 的 `texturePackPath`，可通过缩短/清空或改为单行目录名恢复。协议保持 v26，engine ABI v6、区块 schema v9、玩家 schema v7、世界 metadata v2、`companions.ai` schema v4 与 benchmark scenario v19 均不变。
