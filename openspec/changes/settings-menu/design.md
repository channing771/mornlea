...
设置页和配置加载共用 `MaxTexturePackPathBytes = 1024`。上限按 UTF-8 字节计，既符合 ABI 与 UI 固定内存预算，也覆盖 Darwin 可用路径；超长旧值需用户缩短或清空，但配置版本仍为 1。把限制只放在 UI 会让已加载超长值无法编码设置快照；静默截断又会在保存时改写路径，因此选择在加载边界显式拒绝。
...
- [超长旧材质路径从“可解析”变为配置错误] → 上限与 Darwin 路径/ABI 预算同源，错误包含字段名；迁移只需缩短或清空，配置版本不变。
...
1. 先以配置 v1 兼容测试落地 `windowSize` 默认与三预设，以及 1024 字节路径门禁；旧普通配置无需改写。
