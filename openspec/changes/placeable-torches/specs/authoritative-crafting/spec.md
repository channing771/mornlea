...
系统 SHALL 定义十四条稳定固定形状配方，recipe ID、形状、原料、数量和产物 MUST 由服务端定义，客户端不得声明或覆盖这些值。形状以裁边后的宽高与非空格序列表达，匹配遵守 `authoritative-grid-crafting` 的裁边与水平镜像规则：
...
- recipe ID `14`：纵向 2 格、煤炭位于木棍正上方，产出 4 个火把。

单格配方的形状可出现在归一化后网格的任一位置。recipe ID 只用于注册表与 UI 身份，新路径的线上消息 MUST NOT 携带 recipe ID，recipe ID MUST NOT 落盘。recipe `15..18`（木剑、石剑、铁剑、白床）由批次其余功能行在各自变更中追加，追加前查询这些 ID MUST 稳定拒绝。相同初始状态与命令序列经 Memory 和 TCP MUST 得到相同结果。
...
- **WHEN** 系统查询 recipe ID `0`、大于 `14` 的值或尚未由批次其余功能行追加的 `15..18`
- **THEN** 查询 MUST 稳定拒绝且不产生任何产物

#### Scenario: 既有配方编号不因新增而位移

- **GIVEN** 追加 recipe `14` 后的配方表
- **WHEN** 系统查询 recipe ID `1`..`13`
