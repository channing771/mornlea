...
- [x] RED：穷举 0..<`BlockIDMax` 的 `BlockEmission`（发光方块 15、五种火把 14、其余 0、越界 0）与 `BlockLightAttenuation`（八流体 1、其余 0）；五种火把非 opaque/零碰撞/可被射线瞄准；`ItemTorch`=44（集成重订前 43）堆叠 64 且不进食物/工具表；`BlockDrop` 五形态均掉回一个火把；`PlaceableBlockAtFace` 逐面映射与立方体物品不随面变化；`RecipeTorch`=15（集成重订前 14）形状为「煤炭位于木棍正上方」产出 4 火把、recipe 1..14 语义不变、查询 16 及以上稳定拒绝。
- [x] GREEN：core 追加火把枚举 71..75（集成重订前 62..66，门方块 62..70 之后）、`ItemTorch`=44、两张光源表、`PlaceableBlockAtFace`、`BlockDrop` 映射与 `RecipeTorch`；assets 的 `Emission`/`LightAttenuation` 转调 core 并删除重复 switch。
...
- [x] golden：经显式基线更新写入 `torch-night.png`（原始口径 19 → 20 张；集成基线因 main 补齐 `workbench-crafting` 与 F-05 再生为 20 → 21 张），逐图人工复核；其余既有 golden 不由本分支改动（集成期以 main 的 F-05 版本为准）。提交 `feat: present placeable torches`。
