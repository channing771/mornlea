...
- 目标文件/包：`internal/mesh`（`native_input.go`、`native_abi.go`、`registry.go`、`native_input_test.go`、`native_abi_test.go`、`native_capacity_test.go`、`native_parity_test.go`、`greedy_test.go`、新建 `torch_test.go`）；`internal/nativeabi`（`native.go`、`native_test.go`）；`engine/crates/mornlea_engine`（`src/input.rs`、`src/greedy/mod.rs`、新建 `src/greedy/torch.rs`、`src/greedy/torch_tests.rs`、`src/ffi.rs`）；`engine/include/mornlea_engine.h`；`AGENTS.md`/`CLAUDE.md`（仅 engine ABI v7→v8 版本表述的最小同步，`TestBaselineVersionsMatchCode` 转绿为准，两份逐字节相同）。
- 验证命令：`make rust`；`cd engine && cargo test --workspace --locked`；`go test ./internal/mesh ./internal/nativeabi -race -count=1`；`go test ./internal/archcheck -count=1`；`cmp -s AGENTS.md CLAUDE.md`。
