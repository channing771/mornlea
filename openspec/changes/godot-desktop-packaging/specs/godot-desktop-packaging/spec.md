...
Candidate P13 production route authorized by the Godot pilot GO decision. This specification is not live production behavior until the change is independently applied.

## ADDED Requirements

### Requirement: P13 stays a later catalog-replaceable production step

Desktop packaging SHALL keep `apps/mornlea-godot` as the sole project root, and local play MUST reuse the existing login and authoritative simulation path. The existing Rust graphical client MUST remain the default entry until a later explicit switch. Implementation of this requirement is deferred until this change is independently applied.
