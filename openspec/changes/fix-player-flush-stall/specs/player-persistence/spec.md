...
`playerPersistence.Flush` SHALL 在单次调用内对每个玩家至多派发一次冻结重试（retry 类）与一次最新快照（fresh 类）保存；即使玩家快照在 Flush 期间持续变化（恒脏态），Flush MUST 在全部已派发保存完成后返回，不得无界重派。继承自 Flush 开始前的 in-flight 保存 MUST 预占该玩家的 retry 类名额，其等待屏障仍按 `(playerID, revision)` 精确身份匹配。
