...
- **近战攻击**：3 点伤害、20 tick 冷却、水平距离 ≤1.8 且同维；同 tick 先冻结全部意图再统一结算，命中经既有 `applyDamage` 入口；呼吸不扣任何玩家物品。本分支以专属 damage test seam 验证伤害与冷却；剑/夜行者统一 combat settlement 待 A-03 统一战斗落地后收编并删除 seam。
...
- **协议（升版 v29→v30）**：S→C 新增 `HostileSpawn`/`HostileState`/`HostileDespawn` 三类消息（`ServerTick` u64 + count u8 + ≤64 条 record、record 按 ID 严格升序；spawn 携带 ID/dimension/position/yaw/health，state 携带 ID/position/velocity/yaw/health，despawn 携带 ID）；按会话已订阅 chunk 订阅发布（进入视野发 spawn、持续发 state、离开/死亡发 despawn）；S→C 编号取 22/23/24（21 已被 `CraftingState` 占用，实现期以注册表实占空闲位为准，撞号按 A-02 先例重订）；协议版本号由本行升到 v30 并维护旧客户端握手拒绝语义。
...
- **client ABI v9→v10**：本分支在 `engine/crates/mornlea_client` 与客户端边界同步升版（ABI 常量、容量与尺寸对齐、早期拒绝旧动态库），并按 A-02 裁决先例对 `AGENTS.md`/`CLAUDE.md` 做**最小同步**：协议 v30、client ABI v10、`hostile_mobs` v1 三处版本行、两份逐字节相同；benchmark scenario 不动。
- **单一发光/透明度表**：`core.BlockEmission` 与 `core.BlockLightAttenuation` 已由现行 `internal/core/block_properties.go` 提供，本分支直接消费；只新增 `core.BlockOpaque` 单一表（判据与既有 `assets.Registry.Opaque` 一致），`internal/assets`/`internal/mesh` 的不透明谓词改为委托 core。
- **视觉场景构造**：`cmd/mornlea` 追加 `hostile-mob` capture 场景构造（固定夜间火把边缘 8 只夜行者，其中一只受击、一只追逐；无 nametag 断言），插入 `ai-companion` 与 `water-surface-slope` 之间；**自带 golden PNG**（torch-night 先例，场景总数 21→22 口径）。
...
- **协议**：追加三类 S→C 消息；协议版本号 v29→v30 由本行升级。老客户端握手拒绝语义由本行在 v30 维护。
...
- **并行边界**：本行自带协议 v30、client ABI v10、`hostile_mobs` v1、golden 与两份基线文档版本行同步；benchmark scenario、玩家 schema、区块 schema、世界 metadata 不动。与并行行 A-05（床与睡眠）的文件交集仅限 `internal/core` 追加段（`DisplayDayPhase` 的 offset 参数即两者共享契约，本行消费、恒 0）与 `internal/network/registry.go` 追加段，冲突由后合并者 rebase 重订。
...
- 统一战斗契约（同维最近命中、等距按 ID、击退与冷却）由 A-03 交付后收编；本分支只锁夜行者自身伤害入口与冷却。
- 每 tick 1 候选、2 快照、64 上限、13/256 生成 hash 阈值等数值以本 change 为最终契约。
- 床与睡眠（A-05 并行行）经 `core.DisplayDayPhase` 的 offset 窄接口解耦：本行消费 offset（恒 0），不做「靠近拒睡」类夜行者查询；床跨区块原子事务（B-21）仍不在范围。
