...
- [x] 4.5 失败测试：灼烧（露天白昼每 20 tick 扣 1、遮顶/夜间重置）、despawn（>64 格累计 600、≤64 回界清零）、死亡掉落（同 tick 移除、死亡 chunk 环形尝试放 1 腐肉、全满确定性省略）；扩展 `core.FoodValue` 两食物更新后实现
...
- [x] 8.3 独立整分支终审（规格合规 + 上限 + worker 不阻 tick + spawn 重放 + 暗度 oracle + schema 错误矩阵 + 重启 + wire 订阅 + 75-body ABI + 无 nametag + 无版权资源）；终审结论与验证输出摘要写入 `ledger.md`；推送分支开 PR，评审通过后合并 main（含 golden 与基线文档版本行）
