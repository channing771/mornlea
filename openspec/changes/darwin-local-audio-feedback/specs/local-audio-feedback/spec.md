...
图形客户端 SHALL 只在有效本地 UI 操作、成功采掘完成、收到本会话首次有效的 `PlaceBlockSucceeded`、成功进食完成和收到伤害确认这些边界各播放对应的一次本地 cue。预测、拒绝、失败、空白或禁用 UI、旧/重复放置序号和同一确认的重复镜像 MUST NOT 播放 cue。同一权威状态同时首次确认进食完成与伤害时，两个 cue MUST 各播放一次，不得以优先级丢弃任一确认。
