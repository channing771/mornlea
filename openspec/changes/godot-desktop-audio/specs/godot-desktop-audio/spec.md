...
Candidate P11 production route authorized by the Godot pilot GO decision. This specification is not live production behavior until the change is independently applied.

## ADDED Requirements

### Requirement: P11 stays a later catalog-replaceable production step

Desktop audio SHALL play each semantic cue exactly once, and headless or capture paths MUST NOT open an audio device. The existing Rust graphical client MUST remain the default entry until a later explicit switch. Implementation of this requirement is deferred until this change is independently applied.
