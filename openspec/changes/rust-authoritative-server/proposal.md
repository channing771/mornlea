...
Planning only: F2 in [`docs/architecture-target.md`](../../../docs/architecture-target.md). Blocked on completed [F1](../rust-runtime-foundation/proposal.md) compatibility evidence. Completion requires the implementation SHA, executed non-empty tests, corpus coverage, failure-path results and rollback evidence in `ledger.md`; file existence or OpenSpec status is insufficient.
