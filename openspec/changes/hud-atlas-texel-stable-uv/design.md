...
- **为什么能钉住稳定性**：收进后解码界距列边界 ≥ δ − 重归一化噪声；噪声上界 `W·2^-24` 在 `W < 2^16` 纹素（4096 列、约 6.5 万种物品）时 ≤ 1/2048 纹素，安全裕度 >8×。任何采样点与列边界的距离恒 ≥ δ − 噪声 − 采样重分布量 > 0，故 floor 取整结果与扩列无关，既有列解析到的纹素集合不变。
- **为什么是 1/256 而不是更大**：收进会把每个采样点移动 ≤δ 纹素，δ 越大对当前渲染的扰动越大；1/256 已远超噪声上界四个数量级以上，再小会侵蚀裕度，再大会增加贴边界样本翻转风险。
