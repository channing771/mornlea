...
- 2026-08-25 Task 4.1/4.2（收尾门禁）：`gofmt -l .` 无输出；`go vet ./...` 通过；`go test ./... -race` 26 包 ok，`internal/server` 的 `TestCompanionDialogueTerminalCoversFourTerminalStates/Stopped` 在全量并发负载下超时一次（60s 等待预算），单独与包级重跑在本分支与 main 基线均通过（4.97s/3.10s/215.68s）——负载性 flake，与本变更（HUD UV，无因果路径）无关，疑似 E-11 同类的等待预算问题，留待 server 测试基础设施任务；`go test ./internal/archcheck -count=1` 通过；`openspec validate --all --strict --no-interactive` 65/65。SPEC 最终确认轮（ses_fc6c5425bffeveHThYsL3vR73V）结论 PASS，唯一 Minor（tasks.md 宽度集括注措辞）已当场清偿。基线文档裁决：本变更不新增能力与契约版本（协议/schema/ABI/scenario 均不变），`AGENTS.md`/`CLAUDE.md`/`progress.md` 基线段无需改动；行为契约沉淀走 `survival-hud-presentation` 主规格。
- 全部任务完成。
