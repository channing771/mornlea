...
- [x] 1.3 新建 `internal/sim/tuning`，迁移 `Tunables`、默认值、钳制和原子活动快照；让 config 与客户端调试装配直接消费新包，根包只作本阶段必要的临时内部调用。目标包：`internal/sim/tuning`、`internal/config`、`cmd/mornlea` 与相关测试。验证：`go test ./internal/sim/tuning ./internal/config ./cmd/mornlea -race -count=1`。
...
- [x] 2.2 将 fluid、耕地湿度、作物、干耕地退化、火把/床支撑复核及其有界 scratch/队列迁入 `realm`，所有环境写入只通过同一 `realm.Mutation`；保留既有预算、重扫、随机抽样和区块写入顺序。目标包：`internal/sim/fluid*.go`、`crop*.go`、`farmland_*.go`、`torch.go`、`bed.go` 的支持复核声明及关联测试。验证：`go test ./internal/sim/realm ./internal/fluid -race -count=1`，以及现有流体/作物 benchmark 的 record-only 运行。
...
- [x] 4.2 将 `internal/server` 的 engine 装配与会话入口迁至 `runtime`/`contract`，将 config 和客户端调试路径迁至 `tuning`，更新所有其余生产调用方；删除根 `internal/sim` 的全部生产 Go 文件和临时桥接，不保留 alias 或 forwarding API。验证：`go test ./internal/sim/... ./internal/server ./internal/config ./cmd/mornlea -race -count=1` 与全仓旧 `sim` import/符号搜索无遗留。
...
- [x] 5.2 将其余白盒测试与 owner package 一同迁移，比较迁移前后的 Test、Benchmark、Fuzz 和 `t.Run` 清单；更新 `docs/architecture.md` 与 `docs/test-organization.md` 的现行路径说明。验证：`go test ./internal/sim/... -race -count=1`，清单比较为零差异，且 `git diff --check` 通过。
...
- [x] 7.1 先增加会在当前双重所有权下失败的定向测试，再让 `runtime.Engine` 只组合一个 `realm.State` 和一个 `entity.State`；将玩家、伙伴、夜行者、背包、容器、战斗、掉落与生命周期状态收敛到 `entity.State`，runtime 对外方法仅做窄委派和 tick 编排；删除 runtime 内对应集合、镜像字段和双写。保持现有对外 API、tick 阶段、稳定排序、存档和 wire 行为不变。验证：`go test ./internal/sim/entity ./internal/sim/runtime ./internal/server -race -count=1`、`go test ./internal/archcheck -count=1`、`git diff --check`。
- [x] 7.2 先增加 tuning 并发读写只返回完整快照的守护测试、使用 runtime phase seam 的确定性 RED（证明当前 entity physics/浸没会在 tick 中途重读全局 physics tunables），以及 server manager 在显式参数束与冲突全局值并存时服从显式值的测试。新增值型 `runtime.TickTunables` 与 `ActiveTickTunables`：两组独立活动快照各读取一次，不宣称跨组原子事务；`Server.Step` 在暂停早退之后、聊天和伙伴任务之前捕获一次，并把同一参数束传给 manager 与 `runtime.Engine.StepWithTunables`，shutdown 最终 pass 同样复用，既有 `Engine.Step` 只作为直接调用方的捕获 wrapper。runtime 从 simulation 值只投影一次 realm 环境配置；entity 显式接收 simulation/physics 值；physics 新增 `StepWithTunables` 与 `SubmersionFlagsWithTunables`，既有 wrapper 保持非权威调用兼容。权威 entity/runtime/server 路径不得再次调用任一 `ActiveTunables` 或无显式参数的 physics wrapper；不新增 `tuning -> physics` 依赖，不改变默认数值、配置/schema、物理结果、Rust/native ABI 或版本。验证：先运行 `make rust`，再运行 `go test ./internal/sim/tuning ./internal/sim/entity ./internal/sim/runtime ./internal/physics ./internal/server -race -count=1`、`go test ./internal/archcheck -count=1`、既有 Go physics golden/差分测试、`cd engine && cargo test -p mornlea_engine --locked`、权威路径 source guard、`gofmt` 与 `git diff --check`。
- [ ] 7.3 更新所有权文档和架构门禁，重新比较迁移前 Test、Benchmark、Fuzz 及 `t.Run` 清单为零缺失；运行 `make rust`、`gofmt -l .`、`go vet ./...`、`go test ./... -race -count=1`、`go test ./internal/archcheck -count=1`、`openspec validate sim-subpackages --strict --no-interactive`、`openspec validate --all --strict --no-interactive` 与 `git diff --check`。完成新的整 change 规格/质量终审，只有确认 runtime 无实体权威镜像、entity 为单一所有者、每个实际推进模拟的 server tick 对 simulation/physics 各捕获一次并复用同一不可变参数束，且测试入口零缺失后才再标记 change 完成。
