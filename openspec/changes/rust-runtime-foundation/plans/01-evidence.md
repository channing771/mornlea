...
Dependencies: 1.2, 1.3, 1.4. Own new `ORACLE/runner_helpers_test.go` (named `runner_test.go` in the original packet; renamed to satisfy the `docs/test-organization.md` helper-file rule because it contains no test functions), `protocol_frame_test.go`, and `SUPPORT/runtime_corpus.rs` runner additions. `type GoOperation func(CaseSpec, []byte) (Outcome, []byte, error)` and `map[string]GoOperation` exist in test code only. Unknown operation/family is a hard error. `RunCases` enumerates manifest selection, hashes input, invokes the registered function once per declared checkpoint and produces observations from returned values. It never supplies expected outcomes to the callback. The stdlib CLI reconciles/validates existing artifacts; it cannot generate fake execution traces.
