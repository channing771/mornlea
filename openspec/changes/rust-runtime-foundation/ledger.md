...
## 2026-09-21 — 1.5 independent operation runners accepted

- Baseline for implementation: `2b2e8355`. Implementation commits: `427540ce` (Go operation runner plus framing producer over the real `codec.WriteFrame`/`ReadFrame`, Rust `FrozenCase` operation/arguments and behavioral dispatch, the frozen noncanonical length vector `82 00 00 2d`, export gated solely by `RUNTIME_ORACLE_EXPORT_DIR`), `20247188` and `b663f362` (review fixes), then the controller merge `290f63d9` and the fixture alignment `bebc91aa`. Every commit scope was verified by the controller.
- Review loop: the first review returned one Important finding — the node froze `"kind": "rejected"` with `"category": "noncanonical-uvarint"`, contradicting the closed vocabulary in `execution-contract.md` (`kind` is `ok` or `error`; structural categories include `invalid-varint`). Ruling: the implementation conforms to the frozen contract; the contract was not amended. The correction landed in the Go producer, the committed expectation, and the Rust expectation, and `TestCorpusOutcomeVocabularyMatchesExecutionContract` now constrains every `*.expected.json` in the corpus to that vocabulary, so a second drift cannot be inherited silently. The re-review returned spec compliant with no remaining gap and quality Approved.
- Controller merge and its consequences: the manifest fragment was merged into `testdata/runtime-migration/contracts.json` (top-level case entry plus the `protocol.frame` family case list), changing the corpus digest; the inventory, trace-identity, Rust `corpus_frame` and domain identity gates were re-run and pass. The merge exposed the expected common-test-root coupling, which the controller serialized: the Go working manifest stopped injecting the now-frozen case, the shared trace fixture derives each observation from its case's frozen expected outcome instead of hardcoding one case, and the Rust consumer loads the merged case through the frozen index with the pre-merge inline entry and path loader removed.
- Controller housekeeping during this node: the planning artifact `packet-contracts.md` quoted the real Go command literal `停止` with curly quotes and failed `TestOpenSpecLanguage`; it is now a backticked code literal with the value preserved verbatim (`c3c0411a`), and the language gates pass. A stray untracked build binary left at the repository root by a review worker was removed. The node packet's literal filename `runner_test.go` was updated to `runner_helpers_test.go` to satisfy the `docs/test-organization.md` helper-file rule (`0f61ffc9`).
- Adjudicated deviations recorded: the Rust consumer dispatches through the real exported `read_frame` because `read_frame_ref` is a planned API owned by a later node; the helper `frozenOutcomeForCase` panics rather than calling `t.Fatalf` because the shared fixture's signature could not change without editing a file outside the node's ownership, and the path is unreachable while the corpus is intact; classification by English error prose is the accepted trade-off under the no-new-production-export rule.
- Open Minors carried to the final whole-branch review: unbounded `os.ReadFile` of expected outcomes on the export path; per-checkpoint export path collision for multi-checkpoint cases; an ordering comment whose assertion does not pin the case tiebreak; the Rust consumer requiring full input consumption while the Go producer ignores trailing bytes; the mutated-ID assertion proving sensitivity rather than decoding; `EncodedPayloadDigest` never cross-checked; the export-unset test setting an empty value instead of unsetting.
- Architecture skill: no change. Independent Go producers with Rust consumers over one frozen corpus, the closed outcome vocabulary, and controller-side manifest merging are change-local evidence rules already covered by the foundation crate-split and offline-leaf rules.

## 2026-09-21 — 1.6 agent contract evidence accepted

- Baseline for implementation: `18f68124`. Implementation commits: `fbd9626a` (package-local producer in `packages/shared/companion` executing all 154 golden fixture cases through the existing pure Go contract validator, the oracle consumer in `packages/tools/cmd/runtime-oracle`, and 308 frozen corpus files), `571f7920` (review fix), `1e62f284` (merge-coupling repair), and the controller merge `0c0a004b`. Every commit scope was verified by the controller.
- Review loop: the first review returned one Important finding — `agentContractInput` had no representation for the golden fixtures' `value_utf8_hex` form, so the two invalid-UTF-8 cases froze inputs with no value and lost the bytes they validate. The fix added the field (mirroring the golden fixture's own name and `omitempty` semantics), regenerated exactly the two inputs, and added `TestRuntimeAgentContractCorpusInputsAreSelfContaining`, which reads every committed input through its own struct and failed red naming exactly those two files. The re-review returned spec compliant with no remaining gap and quality Approved with no findings.
- Controller merge: 154 case entries were merged into the frozen manifest (156 cases total) together with both agent families' case lists and five source provenance entries each. Before merging, the controller verified every fragment digest against disk (308 asset digests and 10 source digests) and confirmed the provenance is truthful: the HTTP family's evidence genuinely depends on `mcp-v1/schema.json` because the HTTP contract's schema registry maps tool inputs to it, and the previously missing `http-v1/schema.json` and `mine-validation.json` entries were added. The merge exposed the third instance of the common-test-root coupling — the framing tests' working manifest loaded the whole frozen corpus and handed agent cases to a package with no registered agent producer — repaired by scoping the framing working manifest to its family, mirroring the agent node's existing pattern.
- Discovered and executed at HEAD (`0c0a004b`), controller-verified: the producer filter, the oracle filter, both full packages under `-race`, `go vet`, `gofmt -l`, the audit dependency pair, Rust `corpus_frame`, and the domain identity gate. `TestCorpusOutcomeVocabularyMatchesExecutionContract` now covers all 156 frozen expectations (60 `ok`, 94 `error` plus the two framing outcomes; categories stay inside the frozen sets).
- Adjudications recorded: schema rejections that name no version declaration, unknown enum member or non-canonical identity are recorded as `invalid-value` with the exact path/keyword/rule in `fields`, and mine rejections likewise, because the frozen vocabulary has no contract-schema category and inventing one would repeat the vocabulary drift this change already corrected; accepted outcomes record the canonical key set plus schema name, which is acceptable because the full accepted document is the frozen input; the additive `value_utf8_hex` input member is acceptable because the producer and the oracle test are the only decoders and no Rust consumer reads agent inputs; these two families' consumer is the explicit `external:agent-contract` marker plus the Go test target, so no Rust Agent parity is claimed anywhere.
- Open items carried forward: `DiscoverCases` still returns only the framing case and must be extended before the acceptance node can claim discovery rather than local assembly; the manifest-bindings test duplicates assertions already pinned by the existing manifest-consistency tests; `packages/shared/companion` has no directory guide (root guide applies) for the guide node to consider; the trace identity positive-table subtest name `one real framing case at0` predates corpus growth and is now a stale label only.
- Architecture skill: no change. Package-local Go producers with an explicit external consumer marker, and the closed outcome vocabulary, are change-local evidence rules already covered by the offline-leaf and foundation crate-split rules.

## 2026-09-21 — 2.2 domain identity, text and scalar values accepted

- Baseline for implementation: `c0fb153f`. Implementation commit: `eb14f23d` (five files, all under `packages/engine/crates/mornlea_domain/`). The review returned spec compliant and quality Approved with no Critical or Important findings.
- Parity was verified by the reviewer against the named Go sources and re-verified by the controller for the downstream crates: `mornlea_protocol` `runtime_contract` 134 passed and `mornlea_storage` `runtime_contract` 63 passed after the additive domain change, so the domain API growth broke no consumer. This also corrects the implementer report's claim that no other crate depends on `mornlea_domain`: `mornlea_protocol` and `mornlea_storage` both declare it and protocol consumes `Dimension`, `HotbarSlot` and `PlayerInput` across many modules; the conclusion drawn from the claim (no downstream compile risk) was nevertheless correct because no downstream code matches on `DomainError`.
- Rulings recorded: the whitespace and control predicates are explicit character-range comparisons (U+0009..000D, 0020, 0085, 00A0, 1680, 2000..200A, 2028, 2029, 202F, 205F, 3000 and U+0000..001F, U+007F..009F) rather than a Unicode crate, so the crate's empty production dependency set holds and behavior cannot drift with a Unicode-version bump; `try_from_canonical` performs no trim and no normalization because admission owns the trim, which is exactly Go's post-trim invariant; the 128-byte display-name bound is not independently observable because a UTF-8 scalar is at most four bytes, so a 129-byte name necessarily exceeds 32 scalars and the scalar guard fires first in both Go and Rust — the recorded concern is an accurate observation, not a divergence.
- Go oracles pinned: `NormalizeDisplayName` and `PlayerID.Valid` (`packages/shared/core/player_id.go`), `companion.ValidateName` (`packages/shared/companion/identity.go`), and the 1024-byte command and 256-byte speech bounds (`companion.MaxPlanCommandBytes`, `companion.MaxDialogueLineBytes`), byte-counted, with each bound's origin documented at its definition.
- Discovered and executed at HEAD (`eb14f23d`), controller-verified: `identity_values` 13 passed with a real nonzero `-- --list` discovery, `runtime_contract` 10 passed unchanged, `cargo fmt --check` clean, `cargo metadata` still resolving the six workspace members. Zero-test discovery would have been rejected.
- Open Minors carried to the final whole-branch review: `FiniteVec3::try_new` reports the rotation-specific `NonFiniteRotation` variant although the type also holds positions and velocities; compile-time type distinction is enforced by the sealed-trait idiom with the negative case only in a comment, because a `compile_fail` harness would need a dev-dependency this change does not authorize; the 1024/256 literals now exist in several places, forced by the domain-cannot-import-protocol direction, with each site naming its Go origin; the absence of a NONE companion identity holds structurally but has no mechanical test.
- Architecture skill: no change. Explicit Unicode-range predicates and post-trim canonical constructors are crate-local contracts of the domain port.
