...
第一次点击有效格 MUST 只记录当前来源并显示来源轮廓；第二次点击有效目标 MUST 发送恰好一个移动请求（统一视图内的网格移动或既有容器移动）并清除来源；点击产物格 MUST 发送恰好一次产物取出请求。客户端 MUST NOT 因任何点击而本地扣除、增加或移动物品；服务端继续是 inventory、grid、furnace 和 chest 状态的唯一权威。
