...
- **GIVEN** a bounded subtask meets the context-isolation criteria and no explicit model has been requested
...
- **GIVEN** a lower-cost routed subagent produces observable validation failure, unresolved contradiction, or material uncertainty
- **WHEN** the controller decides whether to retry
- **THEN** it MAY escalate one eligible capability or effort step
- **AND** MUST prefer targeted follow-up or verification over restarting unrelated completed work

#### Scenario: Routing balances quality and token efficiency

- **GIVEN** the live host exposes multiple eligible model and effort levels
- **WHEN** the controller routes an isolated task
- **THEN** it MUST consider task risk, context, tools, and validation strength from multiple independent axes
- **AND** SHALL choose the lowest-cost configuration that remains credibly sufficient for code quality

#### Scenario: Round-end router review finds no reusable improvement

- **GIVEN** an implementation round has completed and its routing outcomes have been reviewed
- **WHEN** the evidence reveals neither a stable over-routing correction nor a stable under-routing correction
- **THEN** both project router skills MUST remain unchanged
- **AND** the ledger SHALL record `Model router: no change` with the reason
