...
|---|---|---|
| OpenAI-native | Controller is verifiably ChatGPT/Codex on an OpenAI model | Standing project authorization lets the controller use isolation-first direct, native-subagent, or external-agent execution without a separate per-task user request |
...
OpenAI-native mode does not prescribe a fixed agent topology. The project policy itself is standing authorization to delegate, so silence about subagents in an individual task does not force main-agent-only execution; an explicit user prohibition or a higher-priority runtime restriction still controls. To bound token and coordination cost, at most two native or external subagents may run concurrently. A third independent item is queued, performed by the controller, or started after a slot closes.

Execution is isolation-first rather than main-agent-first. A fresh worker is preferred for bounded repository discovery, multi-file reasoning, specialized review, or a long tool trace when retaining those details would cost more main-agent context than a concise handoff and integration. Direct execution remains appropriate for tiny tasks, work tightly coupled to the controller's current edit, or work cheaper to finish than to specify. A free slot, independent file ownership, or possible speedup is not sufficient by itself. Each worker receives a concise task brief with the required evidence, paths, constraints, ownership, integration point, and validation; native workers use a fresh or minimal fork rather than the full conversation by default. The controller records the isolation reason and expected integration in the change ledger, but that record is not a request for user approval unless the planned action itself requires approval.

When isolation is justified, the controller uses the project-owned `adaptive-model-router` before each new delegation. Live capability discovery comes from the actual invocation surface; routing applies user constraints and the router's OpenAI ceiling, then evaluates reasoning difficulty, consequence, context breadth, tool horizon, modalities, validation strength, worker bootstrap cost, and user cost/speed/quality priorities. Code quality and token efficiency are coequal: the result is the lowest-cost backend/model/effort that is credibly sufficient, not the cheapest option regardless of risk or the highest option regardless of verification strength. Routine translation, extraction, deterministic edits, and narrow validation use a low eligible tier; ordinary coding and bounded synthesis use a mid tier; high tiers are reserved for ambiguous, consequential architecture or correctness work. Except for explicit project ceiling identifiers and the verified Z Code bridge target, the project skill does not copy a model catalog or hard-code currently exposed model names. Existing agents are not restarted solely to downshift them; future independent delegations use the routed choice.

Z Code is integrated through the selected `adaptive-model-router` skill's `scripts/zcode-agent.mjs`, because the current Codex native agent schema exposes a closed OpenAI model enum and cannot be extended by repository text. The bridge runtime is packaged inside both synchronized skill copies rather than a root-level project script. It has three operations:

```text
probe -> verify the installed CLI and enabled desktop GLM-5.3 route without inference
run   -> create a fresh external isolated session from a prompt or stdin brief
send  -> resume a returned session ID for targeted multi-turn follow-up
```

The bridge reads the existing desktop configuration locally, passes the API key and base URL only in the child process environment, selects `builtin:bigmodel-coding-plan/GLM-5.3`, and normalizes the CLI JSON into backend, provider, model, mode, session, response, usage, and projection fields. Known credentials are redacted from provider failures. Tests override the CLI and config paths with fixtures, so validation never consumes the user's model quota. Editing modes require a separate worktree or explicit non-overlapping ownership; `plan` is the default for discovery and review. The controller remains responsible for validating and integrating the result.
...
- Pass `GLM-5.3` to the native Codex subagent model field: the current runtime rejects identifiers outside its enumerated native surface, so repository policy cannot make that registration real.
- Copy the Z Code desktop credential into repository configuration: this would create secret duplication and unnecessary persistence; the bridge reads it locally and redacts failures instead.
- Use a new Z Code process for every follow-up: this would replay context and waste tokens; the bridge exposes the CLI session ID and resumes the same isolated worker.
...
- [Excessive delegation can waste tokens and coordination time] → Use isolation-first routing only for independently briefable work whose main-context retention cost exceeds handoff cost, keep trivial/tightly coupled work direct, account for worker bootstrap cost, and enforce a two-subagent concurrency ceiling.
- [An external Z Code worker may be mistaken for a native Codex model] → Label the bridge and ledger route as external, probe it separately, and never pass `GLM-5.3` to the native model enum.
- [A worker could expose provider credentials or collide with controller edits] → Keep credentials in the child environment with redacted failures, and require isolated worktrees or exclusive non-overlapping files for editing modes.
