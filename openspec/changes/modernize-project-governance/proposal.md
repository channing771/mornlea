...
- Make OpenAI-native execution isolation-first. Prefer a fresh agent for bounded discovery, multi-file reasoning, specialized review, and long work traces when the saved main-agent context outweighs handoff cost; reserve direct execution for tiny or tightly coupled work, do not delegate merely for parallel speed, and never run more than two subagents concurrently.
- Before a subagent delegation, route the subtask through synchronized project-owned `adaptive-model-router` skills using verified live invocation surfaces, the configured OpenAI ceiling, and a multi-axis assessment of complexity, consequence, context, tools, validation, worker bootstrap cost, and user priorities. Treat code quality and token efficiency as coequal objectives; do not default every subagent to the highest model or under-route risky work merely to save tokens.
- Add a credential-safe Z Code CLI bridge that exposes the locally configured `GLM-5.3` worker as an external isolated agent with read-only probing, fresh sessions, session-ID follow-up, and machine-readable results. The bridge does not claim to extend the current Codex native model enum.
...
- Gates and tooling: `packages/audit`, `scripts/agents`, the Z Code agent bridge and its contract tests, Makefile, and CI checks for language, pairing, revisions, current version claims, orchestration policy, and visual ownership.
