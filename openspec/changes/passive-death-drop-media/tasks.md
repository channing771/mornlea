...
- [ ] 7.6 步频位移协调：摆动相位按行进距离推进（固定步幅），速度差分驱动，静止回中（`packages/client/render`；验证 `-race` 全绿 + GIF 目检步态）
- [ ] 7.7 牛肉竖立旋转：薄片竖直绕 Y 旋转 + 浮沉，不平躺（`packages/client/render`；验证 `-race` 全绿 + GIF 目检）
