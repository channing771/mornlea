...
- 受影响实现：`engine/crates/mornlea_client`、`engine/include/mornlea_client.h` 与 `internal/client`；Task 实现与整分支终审均已完成，whole-branch final review findings 的 fix wave、final Rust baseline release/ABI/visual 验证以及 independently reviewed server parity test 修复均已通过 scoped final re-review。client ABI v12 当前事实已同步到 `rust-client-render-cutover` 主规格与 active OpenSpec context；change 保持 active，等待 archive。本结论不代表 archive 或 merge 授权。
