...
The project adds mirrored project-owned orchestration skills:
...
|---|---|---|
| OpenAI-native | Controller is verifiably ChatGPT/Codex on an OpenAI model | Standing project authorization lets the controller use isolation-first direct or native-subagent execution without a separate per-task user request |
...
OpenAI-native mode does not prescribe a fixed agent topology. The project policy itself is standing authorization to delegate, so silence about subagents in an individual task does not force main-agent-only execution; an explicit user prohibition or a higher-priority runtime restriction still controls. To bound token and coordination cost, at most two native subagents may run concurrently. A third independent item is queued, performed by the controller, or started after a slot closes.

Execution is isolation-first rather than main-agent-first. A fresh worker is preferred for bounded repository discovery, multi-file reasoning, specialized review, or a long tool trace when retaining those details would cost more main-agent context than a concise handoff and integration. Direct execution remains appropriate for tiny tasks, work tightly coupled to the controller's current edit, or work cheaper to finish than to specify. A free slot, independent file ownership, or possible speedup is not sufficient by itself. Each worker receives a concise task brief with the required evidence, paths, constraints, ownership, integration point, and validation; native workers use a fresh or minimal fork rather than the full conversation by default. The controller records the isolation reason and expected integration in the change ledger, but that record is not a request for user approval unless the planned action itself requires approval. Editing modes require a separate worktree or explicit non-overlapping ownership. The controller remains responsible for validating and integrating the result. Existing agents are not restarted solely to change their model.
...
Directory-scoped agent guidance is part of the architecture decision aid. Treat the repository root, top-level modules or packages, and subtree roots with an independent ownership, dependency, lifecycle, or validation boundary (or coordination role across packages, entry points, or asset classes) as important directories. Each important directory keeps one concise `AGENTS.md` at its boundary with its purpose, directory map, ownership/dependency boundaries, entry points, lifecycle constraints, and focused validation. Creating, reorganizing, or materially reassigning one of these directories updates the guide in the same change. A directory without an independent invariant inherits its parent guide; guides are not generated merely for symmetry. The exact filename is `AGENTS.md`; established `CLAUDE.md` files remain thin imports and never become a second source of rules.
...
- Create a guide in every directory: this would add repetitive files and create synchronization noise where no independent invariant exists.
- Use lowercase `agent.md`: that would introduce a second convention beside the repository's existing ancestor-chain `AGENTS.md` lookup.
