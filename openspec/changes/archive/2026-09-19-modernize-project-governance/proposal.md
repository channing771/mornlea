...
- Establish directory-scoped agent guidance: important directories maintain a concise `AGENTS.md` describing their purpose, map, boundaries, entry points, lifecycle constraints, and focused validation; directory creation or responsibility changes update that guide in the same change, while directories without independent invariants inherit their parent.
...
- Project skills: new mirrored `mornlea-implementation-orchestration` and `mornlea-architecture` skills for Codex/Claude, plus updates to the mirrored `visual-baseline` skills. The architecture and orchestration skills carry the directory-scoped guidance rule.
