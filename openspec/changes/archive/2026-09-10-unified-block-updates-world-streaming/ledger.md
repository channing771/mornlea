...
- Task 4.1: complete (commits 6e3de9ea..36e99a94：骑手 2c18a978 死 helper 删除/3fc51358 companion_placement 透传/0dd8ff0b backup 过滤 `.create-*`（TDD 先红后绿）+ 36e99a94 perf 取证；聚焦评审 PASS 零发现——SHA-256 亲测一致、291 测试函数集合前后一致、数值换算逐项核对)。全量门禁零失败零修复：gofmt 空、六模块 vet、`make rust`、`make test-race` 六模块 46 包全绿、audit、openspec strict 105/105；`make visual-check` 30 景零差异；v23 Memory record-only 取证入 `docs/notes/perf-baseline.md`（SHA-256 `c6dc6ce8…573c8`、M2 与历史 M5 跨硬件定性对照声明、未提升 perf-baseline.json）。`make build` 尾部可选资产包拷贝失败为预存在环境条件（未 provision，未顺手修）。proposal 补「延期与放弃」节誊入全部开放记档项（e3e57ea6）。**全部任务组完成，进入整分支终审。**
- Ruling: 终审 I-1 修复轮两处有意语义变化接受 — (a) region 文件复制经缓存句柄读锁与同 region 保存/Compact 提交完全串行（`Region.CopyTo` + `copyRegionBackup`，双 bank 撕裂窗口关闭），坏 region 文件时 Backup 整体报错而非复制坏字节（与 `ChunkKeys` 防洗档纪律一致）；两个既有停靠测试按新串行语义改写（原临时文件过滤与 ChunkKeys 不阻塞意图保留）。(b) 「反activate」错字另存 4 处（entity 3 + realm 测试 1）——术语统一记为顺延小修，不在本 change 扩大。
- Final whole-branch review: 附条件 GO → 条件已满足 (commits a297c001..b8c39340：d69af423 fix(storage): serialize backup region copies with concurrent saves + b8c39340 docs(realm): clarify reactive activation wording；聚焦复审 PASS 零发现——锁序无倒置、路径解析双向校验、refs defer 无泄漏、4 测试旧代码红灯判别力成立)。终审 M-2（evict/Close 在 regionMu 内做文件 I/O 的短暂阻塞——worker 侧既定形态）、M-3/M-4（perf-baseline.md 主段 v22 表述与 progress.md 条目——归档阶段同步）记档。**开发完成：分支 feat/unified-block-updates-world-streaming @ b8c39340，未推送；全量门禁（gofmt/vet/rust/test-race 六模块 46 包/audit/openspec strict 105/105/visual-check 30 景零差异）在 36e99a94 基线全绿，修复轮后 storage/.../-race、realm 定点、audit、gofmt 复验绿。合流与归档待用户决策。**
