...
> 标题沿用历史名；本 change 起正式清单为 25 项（新增 `grass-closeup`），语义以下述断言为准。

- **GIVEN** 完整无窗口 capture 场景清单
- **WHEN** 检查全部场景名称与顺序
- **THEN** 清单 MUST 恰好包含本 requirement 列出的 25 项，且顺序与之逐项一致
...
- **WHEN** 显式更新视觉基线
- **THEN** 系统 MUST 按既有完整顺序重新生成全部 25 张正式 golden（含新增 `grass-closeup`）
...
- **AND** `grass-closeup` MUST 紧随 `target-block-feedback` 且先于 `oak-grove`，`water-surface-slope` MUST 位于 `ai-companion` 之后，`main-menu` 与 `settings-menu` MUST 依次相邻并位于 `far-horizon` 之前，`far-horizon` MUST 是倒数第二个场景，`water-underwater` MUST 是唯一末场景
...
- **AND** `grass-closeup` MUST 紧随 `target-block-feedback` 且先于 `oak-grove`
...
- **WHEN** 显式更新视觉基线
- **THEN** 系统 MUST 重新生成全部 25 张正式 golden（含新增 `grass-closeup`），并只提交实际场景文件
...
- **AND** golden 目录 MUST 存在 `torch-night.png`，正式 golden 总数 MUST 恰好为 25 张

#### Scenario: 未受影响场景 golden 逐字节不变

- **GIVEN** 本变更只新增 `grass-closeup` 一个场景，不改动既有夹具、相机与渲染链路
- **WHEN** 运行 capture 并与本变更合入前的 golden 比对
- **THEN** 既有 24 张 golden 的 PNG 字节 MUST 逐字节不变
- **AND** golden 目录 MUST 恰好有 25 张 PNG（24 张既有 + 新增 `grass-closeup.png`）
...
`grass-closeup` SHALL 为短草提供近景图片基线：确定性手工夹具在草地支撑上立短草列，固定正午与固定近景机位，画面 MUST 呈现至少一株可辨识的短草（交叉斜面、四 quad cutout、上缘透空、贴地生长），经与交互客户端相同的完整呈现链路收敛后无窗口抓取。既有双阈值 MUST 保持不变。
