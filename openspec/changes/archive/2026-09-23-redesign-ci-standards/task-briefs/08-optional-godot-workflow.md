...
The exported-runtime probe defaults to the same cached-editor resolver used by other headless Godot checks (`scripts/godot/godot.sh`), not `/Applications/Godot.app` alone. `MORNLEA_GODOT_BIN` remains an explicit override. This keeps `fetch.sh` and the export probe on one cold-cache path.
