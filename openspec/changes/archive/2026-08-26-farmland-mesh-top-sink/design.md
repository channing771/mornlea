...
- `materials-showcase` 夹具追加干/湿耕地各一个 2×1 列，不移动任何既有方块。落位决策（Task 2 实现期定量裁决）：不放 z=-8 既有柱网格（顶面俯角 ≈7°，1/16 下沉为亚像素噪声不可辨），改放草带左缘同层近相机处 z=-1（俯角 ≈17°，下沉恰 1px 可辨，且与草缘形成相邻 1px 台阶对照）——其余 19−1=18 景 golden 逐字节不变的前提是本场景独占其 golden。
