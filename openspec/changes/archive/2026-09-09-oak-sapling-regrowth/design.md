...
- D5 树形来源：新增 engine ABI `mornlea_tree_blocks`（ABI v10→v11）。输入 28 字节：`MTB1` magic(4) + layout u32(4) + seed i64(8) + x/y/z i32(12)；输出 `count u32` + 每条 8 字节 `[dx i8][dy i8][dz i8][reserved u8][block u16 LE][reserved u16]`，记录上限 `128`（最坏普通橡树 65 条）。几何只取普通橡树家族：高度 `5 + hash%3`、蓬松位由哈希给出、树冠沿用既有普通层形（`-2/-1` 去角 5×5、`0` 3×3、`1` 十字、`2` 蓬松十字），半径 ≤ 2、无分杈、无珍异巨树。参数由独立冻结 salt 从 (世界种子, 根坐标) 派生，与世界生成的 8×8 候选格网格无关，因此 `GenerateChunk`/`BaseBlockAt` 逐格不变、worldgen golden 字节不变。
- D6 空间校验：树形几何的每一格必须在世界高度范围内且当前为 `AirID` 或 `ShortGrassID`；树苗自身格是树干底格，按定义不参与占用校验。任一其它格不满足即放弃本次生长（树苗保留）。覆盖短草零掉落（与短草被流体清除同口径）。
