...
The final code reviewer found a further compressed-stream boundary after the first repair round: libzstd's one-shot decompression ignores the context's `WindowLogMax` parameter. A frame with a declared 128 MiB window and only an 86 KiB decoded payload passes Rust while the Go v45 decoder's 2 MiB memory limit rejects it. Before either Rust decompression entry point, scan each bounded zstd frame without allocating, use libzstd's frame-size parser to locate frame boundaries, and inspect the standard frame header. A single-segment frame's declared content size is its window; other frames use the zstd window-descriptor formula. Reject a declared window above 2 MiB as `Integrity`, the compressed-stream failure class, before calling the bulk decoder. Keep the owned context lifecycle and byte format unchanged. This preflight applies to public one-shot and owned session decoding, including later frames if the compressed bytes contain more than one. A paired Rust/Go mutation of the existing Go fixture pins this interoperability boundary.
