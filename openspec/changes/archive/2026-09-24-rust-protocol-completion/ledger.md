...
**Status:** complete in the isolated integration branch. The Rust red test mutated the Go fixture's frame descriptor from `0xa4` to `0x84`, inserted `0x88` and corrected the envelope length. Baseline one-shot decode returned the complete revision-19 snapshot instead of `Integrity`; the owned path shared the same missing gate. The paired Go test rejected that 128 MiB declaration. Production Rust now scans bounded zstd frames before either bulk decoder call, deriving the single-segment window from frame content size and other windows from the descriptor. It rejects over-2 MiB declarations as `Integrity`; it also checks later concatenated frames without allocating proportional scratch. The configured context limit stays as secondary protection.

**Red → green and boundary:** Rust one-shot and owned decoders now reject windows one eighth above 2 MiB and at 128 MiB while both accept exactly 2 MiB; the Go production decoder makes the same three decisions. `chunk_snapshot::tests` 3/3, `protocol_snapshot` 12/12, `runtime_contract` 134/134 and `protocol_corpus` 24/24 passed. Focused Go snapshot, manifest and protocol closure tests passed; Clippy `-D warnings`, formatting and unchanged runtime-migration asset checks passed. No fixture, compressed output, dependency or version changed. Rollback is this node's one scoped commit. **Architecture skill: no change**; the project already records bound-before-decompression, and this is a libzstd-specific enforcement detail.

## Node 5.6 final acceptance — 2026-09-24

**Status:** complete on the isolated integrated branch. The independent protocol reviewer rechecked the zstd-window repair at `c52256b3` and reported no actionable findings. The independent evidence review of the merged corpus, key identity and cross-language fixture also reported no actionable findings. All five final-review findings are closed; no protocol version, ABI, save schema, live authority or production Go decoder changed. The 45 non-protocol F1 gaps remain explicitly outside this change.

**Integrated gates:** `make rust-check` exited 0; `go test ./packages/audit -count=1` passed; `make test-race` exited 0 across all six Go modules, including `packages/server/server`, the 337.592-second `packages/tools/cmd/runtime-oracle` race suite and `packages/audit`. The network subtree also passed an explicit `go test ./packages/shared/network/... -race -count=1`. The full race run executed the oracle package afresh, so a second identical `-race -count=1` oracle run would add no distinct behavior. `openspec validate --all --strict --no-interactive` passed 127/127 before and after canonical spec sync; `git diff --check` and the tracked runtime-migration corpus check passed.

**Concurrent-gate incident:** the first `make dev-check` overlapped with the full race run and failed on the existing server integration tests `TestCompanionInteractionMemoryTCPParity` (position distance 1.29 above its 0.75 tolerance) and `TestFluidChangesBroadcastOverExistingChunkChannel` (the far mirror cell had not received its fluid). No production Go server/client/shared source changed from main. Both named tests passed alone and in five serial repetitions. After the race run completed, a fresh serial `make dev-check` exited 0, including the server package (79.401 seconds), all six Go short-test modules, Rust format, Clippy and workspace tests. The failed concurrent attempt is retained as evidence and was not accepted as a green gate.

**Spec and provenance:** `6ad7fc98` synchronized exactly five protocol requirements into `openspec/specs/rust-runtime-foundation/spec.md`; all three main region requirements remain exactly once. The 456-byte Rust-produced fixture still hashes to `aab7b542775cc9b5351907f6c89da1881f5e9a0949356224afe50716269e34d6`. The merged `step.rs` manifest SHA remains `188cb73517c29e471afc2733a0c49bced834087ed04871ca408dabf5b7446aac`; the protocol `source_revision` and Go `BaselineSourceRevision` remain the common Go producer baseline `b6043f004176055a2e39a98508b662691c3e4ef7`. The original dirty user checkout and the archived region change remain untouched.

**Integration and rollback:** archive this completed change as `2026-09-24-rust-protocol-completion`, then fast-forward `main` and push it only if the remote is still the reviewed baseline. If the remote advanced, integrate and rerun affected gates before fast-forwarding. Revert the scoped repair commits or the final merge if later acceptance fails; do not overwrite the original checkout. **Architecture skill: no change**; the reviewed rules are already in the synchronized architecture skill, and the zstd preflight is a protocol-local implementation detail.
