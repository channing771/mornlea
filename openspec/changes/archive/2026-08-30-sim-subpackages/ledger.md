...
## Task 7.3 Ownership Documentation, Guard, And Inventory Finalization

- Implementer: fresh implementer `/root/sim_finalization_implement`; bounded read-only documentation, inventory, and `go/types` design analysts informed the implementation, and independent whole-change SPEC and QUALITY reviewers performed the final audit.
- Implementation and integration lineage: initial documentation, compatibility inventory repair, and architecture guard commit `78748c8a`; main specification synchronization `c827f0ad`; normal non-destructive merge `2129b150` of then-current `origin/main` commit `a23833f9`; repair commits `8c13e479`, `5603ade5`, and `86419c8a`. The merge preserved all upstream pathfinding, persistence, fluid-engine, WebView, archive, documentation, and guard work while retaining the ownership and tick-tunable convergence. `git merge-base --is-ancestor a23833f9 HEAD` passed, and the integrated tree retains protocol v32, player schema v8, chunk schema v9, world metadata v3, `companions.ai` schema v4, `hostile_mobs` schema v1, engine ABI v9, client ABI v12, and benchmark scenario v20 with no ABI, wire, schema, persistence, or version rollback.
- OpenSpec lineage ruling: upstream archived `sim-subpackages` while this branch was being integrated, before the post-review ownership repairs were complete. Repair `5603ade5` restored that directory with a normal `git mv` so the repaired change remained auditable as exactly one active change and zero same-name archives; the already-synchronized main repository specification was preserved. Consequently `openspec validate sim-subpackages --strict --no-interactive` genuinely validated the active repaired change rather than relying only on the all-spec gate. Archive remains a separate control-session action.
- Ownership and tunable closure: current guides and architecture documentation identify runtime as the sole serial tick orchestrator composing exactly one `realm.State` and one `entity.State`; entity is the sole player, companion, hostile, inventory, container, combat, drop, and lifecycle owner; runtime has no entity authority mirror. Runtime creates one realm mutation and commits it once. Normal server simulation captures `TickTunables` after lifecycle/pause early returns and before chat/task work, and shutdown's final pass does the same; each pass reuses one value bundle through manager/runtime/entity/realm/physics. Simulation and physics snapshots are each read once but do not form a cross-group atomic transaction. Explicit physics APIs are authoritative, while implicit wrappers remain compatibility boundaries for non-authoritative callers.
- Baseline inventory against immutable Task 1.1 artifacts at `cc416295`: top-level Test/Benchmark/Fuzz entries are `606 → 679` raw occurrences and `606 → 675` unique names, with the four cross-package duplicate occurrences reported rather than hidden and zero missing baseline entries; package-qualified observed subtests are `287 → 415`, zero missing; source `t.Run` call sites are `126 → 169`, and normalized baseline argument expressions and observed labels both have zero missing entries. The immutable baseline set is therefore preserved item by item; authorized additive ownership/tunable guards, current combat regressions, and tests integrated from main are reported separately rather than being used to offset a missing baseline name. This zero-missing plus separately-audited-additions ruling is the accepted preservation interpretation after approved post-baseline feature integration, not a claim that the enlarged current set has no additions.
- Compatibility entries execute real assertions. The five restored `TestMelee*` entry points run the complete current combat/mining fixtures, and `TestPlayerMeleeTarget` retains all six historical labels with live target-order, exclusion, obstruction, fluid, and chunk-readiness assertions. They are not empty aliases or inventory-only shells.
- Clean implementation-HEAD T3 at `86419c8a`: `make rust` passed in 0.48s; `gofmt -l .` produced zero output in 0.20s; `go vet ./...` passed in 0.54s; `go test ./... -race -count=1` passed without starting or focusing a game window in 216.96s, including `internal/server` in 195.657s; `go test ./internal/archcheck -count=1` passed in 5.51s; named strict validation passed in 1.50s; all strict validation passed 79/79 in 1.49s; `git diff --check` and clean status passed.
- Final SPEC review by `/root/sim_final_spec_review`: PASS with 0 findings. The reviewer independently ran the complete archcheck, verbose synthetic authority guards, named and all strict validation, the package-qualified Test/Benchmark/Fuzz inventory comparison, the `origin/main` ancestry check, and `git diff --check`; it confirmed active 1/archive 0, inventory missing 0, no production, simulation-test, or main-spec edit in the final repair, and a clean worktree.
- Final QUALITY review by `/root/sim_final_quality_review`: PASS with 0 Critical, 0 Important, and 0 Minor findings. Targeted evidence included the authority tests at `-count=3`, the same group with `-shuffle=on -count=5`, all 53 synthetic cases individually under `-v`, complete archcheck, `go vet ./internal/archcheck`, scoped gofmt, named/all strict validation, diff check, and clean status. It confirmed actual `types.Var.Type` analysis covers helper, IIFE, assertion, slice, selector, index, dereference, generic, nested owner, and mutable-holder forms while accepting value-only holders and the unrelated production sort comparator.
- Guard ruling: the material name-based storage and mutation-binding limitation is closed through cached export data plus `go/types` type/object identity shared by real-tree and synthetic analysis. The guard intentionally does not claim whole-program SSA: it proves the current direct deterministic `StepWithTunables`/commit path, rejects local helper shadowing, nested control, `go`/`defer`, tracked-binding closure capture, and binding-address escape, but does not prove that every arbitrary future ordinary call, alias, container, or channel cannot retain a mutation. That boundary is explicit, conservative for the current source shape, and accepted by the final reviewers.
- Repair rounds: 3 after the initial implementation and main integration. Final ruling: every in-scope Task 7.3 and finalization finding across inventory, integration, documentation, guards, and auditability is closed; Task 7.3 and all 16 change tasks are approved complete. Previously documented out-of-scope debt remains governed by its existing rulings. The change remains active for the control session to inspect and archive.
