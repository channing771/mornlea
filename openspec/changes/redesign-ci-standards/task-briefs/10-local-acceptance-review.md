...
- 6.2.3 owns `.github/workflows/godot.yml`, `packages/audit/godot_entrypoints_test.go`, `scripts/godot/python-runtime-check.sh`, a focused `packages/audit/godot_toolchain_test.go` cache-resolution test, and the paired `docs/continuous-integration.md` / `.zh.md` with their manifest revision; this brief and `task-briefs/08-optional-godot-workflow.md` are controller-owned planning inputs. First extend the exact path/order audit to require `packages/client/client/**`, `packages/client/render/**`, `make godot-build` after editor/Python materialization, and `scripts/godot/python-runtime-check.sh --exported --offline` after smoke; observe audit failure. Then update both push and pull-request filters and the runtime steps. Existing mutation tests must reject removal of each added path/step. A cold-run export probe must resolve the checksum-verified cached editor through `scripts/godot/godot.sh` by default while retaining the explicit `MORNLEA_GODOT_BIN` override; test that behavior before changing the probe. Update both language guides to name producer and export qualification. The optional job remains a leaf and every added command must remain a hard failure.
