...
Candidate P14 production route authorized by the Godot pilot GO decision. This specification is not live production behavior until the change is independently applied.

## ADDED Requirements

### Requirement: P14 stays a later catalog-replaceable production step

The default client MAY switch to Godot only after a stable release stage, and client ABI v19 MUST remain until a later retirement change with a rollback package. The existing Rust graphical client MUST remain the default entry until a later explicit switch. Implementation of this requirement is deferred until this change is independently applied.
