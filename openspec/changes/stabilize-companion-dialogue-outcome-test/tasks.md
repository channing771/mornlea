...
- [x] 1.1 记录 CI run `32619746660` 的已知 RED、运行环境、失败断言原文、`0.05s` 耗时及其偶发性质。
- [x] 1.2 保留单核 `-race -count=10` 的 18/30 聚合辅助复现观测；因未留存逐次失败断言与耗时，不将其作为正式可审计归因或频次。指定 `-count=100` 仅尝试执行但未留存最终 stdout/stderr，故不声称其有实际结果。
...
- [x] 3.3 在 ledger 区分正式 CI RED、Task 1 辅助复现观测、Task 1 未捕获的 `-count=100` 与 Task 2 后续独立 RED，并记录修改后压力样本和共享门禁退出码；不宣称测试“绝不 flaky”。
