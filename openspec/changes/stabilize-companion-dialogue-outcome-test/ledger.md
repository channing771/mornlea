...
| 本地预检 | `make rust` 退出 0。 |
| 本地压力 | 指定的 `GOMAXPROCS=1 go test ./internal/server -run '^(TestCompanionDialogueOneInFlightPerCompanion|TestCompanionDialogueStaleOutcomeDiscarded|TestCompanionDialogueGenerationBumpDiscardsOutcome)$' -race -count=100` 已启动；此会话的 30 秒工具收集窗口截断了其最终 stdout/stderr，故不虚构频次。相同命令的 `-count=10` 退出 1：30 个子测试中 18 个失败（OneInFlight 4、StaleOutcome 7、GenerationBump 7），均为放行后固定 tick 耗尽仍 `inFlight=true` 或 effects=0。 |
