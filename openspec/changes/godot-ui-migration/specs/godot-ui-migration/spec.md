...
Candidate P10 production route authorized by the Godot pilot GO decision. This specification is not live production behavior until the change is independently applied.

## ADDED Requirements

### Requirement: P10 stays a later catalog-replaceable production step

A later UI change SHALL keep token, hit, and confirmation semantics aligned with the Go mirrors, and MUST NOT write tracked UI goldens until an approved producer handoff. The existing Rust graphical client MUST remain the default entry until a later explicit switch. Implementation of this requirement is deferred until this change is independently applied.
