...
Candidate P12 production route authorized by the Godot pilot GO decision. This specification is not live production behavior until the change is independently applied.

## ADDED Requirements

### Requirement: P12 stays a later catalog-replaceable production step

Tooling ownership MAY move to Godot only after an approved producer handoff that names old and new producers, and MUST NOT relax visual thresholds. The existing Rust graphical client MUST remain the default entry until a later explicit switch. Implementation of this requirement is deferred until this change is independently applied.
