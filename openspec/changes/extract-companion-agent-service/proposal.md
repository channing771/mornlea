...
- Go 在权威 tick 边界冻结以伙伴脚下整数格为中心、水平 ±16/垂直 ±8 的固定 terrain projection；`query_terrain` 与 `mine` validator 只读该投影，隐藏在 256 条暴露方块裁剪之外的目标仍按冻结方块精确判断，未加载范围不得回读 live world。方块/物品英文 machine name 由 Go core 单一注册表提供，未知编号或名称 fail closed。
