...
5. **跨语言动作编号契约（控制会话钉死，T2/T3 共同遵守）**：「返回游戏」= 8、「退回主菜单」= 9，延续主菜单按钮动作表 1..7 之后且互不重叠；动作 8 仅由「返回游戏」按钮产生，Esc 关闭由 Go Esc 栈暂停档裁决，Rust 侧不合成 Escape 动作——宿主 winit 泵同一帧既更新键位快照又入队 UI 键事件，Rust 再合成会把开层当帧回声成关层（winit 泵同帧回声缺陷，终审 B1）；设置页合成先例仅适用于菜单相位。Task 2 在 Rust 侧建立常量并用测试钉住；Task 3 在 Go 侧建立同值常量（沿用 `menuAction*` 命名族）并消费事件。
6. **golden 零变化承诺**：暂停页不出现在任何 capture 场景中，视觉基线不动；阶段 4 以 `make visual-check` 断言不变。
...
| `mornlea_client` | 暂停页 render 纯函数测试（按钮列、注明行有无两个分支）、Escape 键事件不产生动作、按钮→动作映射、layout v4 解码回归 |
