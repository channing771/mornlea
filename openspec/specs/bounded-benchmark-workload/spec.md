...
> 标题沿用历史名（openspec 的 MODIFIED 漂移守卫不支持 Scenario 改名）；本变更后该场景语义为 21:22，历史 `20:21` 随 producer 升到 scenario v22 退役。

- **GIVEN** 一份 scenario v21 基线与一份 scenario v22 当前报告
- **WHEN** 比较器以显式 `21:22` 迁移参数运行
- **THEN** 比较器 MUST 校验报告完整性与硬件身份并跳过跨 workload 的相对回归判定
- **AND** 任何其他迁移参数（含已退役的 `20:21`、`19:20`、`18:19` 与 `17:18`）MUST 失败
