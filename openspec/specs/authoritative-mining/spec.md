...
- **GIVEN** Ready 玩家持续 primary action、未成功命中实体且权威射线命中一个可采掘方块
- **WHEN** 服务端处理第一份有效持续输入并推进一个 tick
- **THEN** 该玩家的权威状态 MUST 报告目标方块、进度 `1` 和对应总 tick

#### Scenario: 持续命中同一状态递增

- **GIVEN** 玩家上一 tick 正在采掘某方块、未成功命中实体且目标方块与选中工具都没有变化
