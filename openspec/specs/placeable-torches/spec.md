# placeable-torches Specification

## Purpose
TBD - created by archiving change placeable-torches. Update Purpose after archive.
## Requirements
### Requirement: 火把物品与五种稳定方块形态

火把物品 `ItemTorch` MUST 是一格堆叠上限 64、不可作为食物/工具的可放置物品，放置出一个火把方块。火把方块 MUST 恰好有五种形态：落地（standing）与四向墙面（wall，分别对应 +X/−X/+Z/−Z 支撑面）。全部五种形态 MUST 零碰撞体、非不透明、非流体，且 MUST 不允许水进入其所在格。火把方块 MUST 发光等级 14。编号为 append-only 最终锁定值（集成重订后）：火把方块 71..75（71=落地、72..75=墙，按 +X/−X/+Z/−Z 顺序，紧随门方块 62..70）、`ItemTorch`=44（`ItemDoor`=43 之后）；既有方块与物品编号 MUST NOT 因本变更位移或重排。
...
- **WHEN** 查询每个结果方块
- **THEN** 五种形态 MUST 分别解析为 71..75 范围内的五个不同方块编号
...
任何权威方块变化（玩家/伙伴采掘、流体/作物阶段替换、放置等）落下后，服务端 MUST 只对**本 tick 已变化**的位置按稳定顺序排序去重，逐一检查其精确六邻居；邻居若是火把且该火把的支撑格就是变化格，MUST 通过既有变更记录把火把写成空气，并生成一枚火把掉落物品。新移除的火把 MUST NOT 被继续当作另一火把的支撑源进行级联（火把零碰撞，不可能成为合法支撑），故不需要通用递归队列。火把移除 MUST 与原变化共享同一批 revision/广播/存档路径。仅改变非支撑邻居 MUST 不影响火把。所在区块掉落物槽容量不足时 MUST 整体保留火把——不写空气、不产生掉落、无半结算——该格的下一次权威变化重新触发复核。复核只读取已加载邻居：邻居所在区块未加载时本轮跳过，不产生读取、写入或失败。
...
#### Scenario: 掉落容量不足时火把保留原位

- **GIVEN** 一枚火把的支撑格在本 tick 被权威采掘移除，且火把所在区块的掉落物槽已全部占用
- **WHEN** 该权威 tick 结束
- **THEN** 火把 MUST 保持原位（不写空气、不产生掉落、无半结算）
- **AND** 掉落容量恢复后，该支撑格的下一次权威变化 MUST 重新触发复核并完成移除与掉落

#### Scenario: 邻居区块未加载时跳过本轮复核

- **GIVEN** 本 tick 某已变化位置的精确六邻居中存在位于未加载区块的火把格
- **WHEN** 支撑复核执行
- **THEN** 该未加载邻居 MUST 被本轮跳过，复核不读取、不写入该格且不产生失败
