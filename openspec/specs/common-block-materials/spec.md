...
新玩家在缺省世界生成下以空初始背包起步时，服务端 SHALL 保证工作台、照明、采掘工具与食物四条关键能力各自存在一条只经自然获取与既有采掘、合成、熔炼规则即可走通的路径。以自然获取集合为起点、按既有规则求值到不动点的价值闭包中，四条能力所需的物品——含食物路径的前置农具——MUST 全部位于闭包内；并且 MUST NOT 存在任何配方或熔炼以「无自然来源且不在闭包内」的物品作为必需输入。取消一次性发放 MUST NOT 把任何既有已交付内容（含熔炉、熔炼产物、铁锭与空桶）推出闭包。
...
- **AND** 不在闭包内的物品集合 MUST 恰由单一显式清单声明，且该清单 MUST 只包含不自然生成的装饰材料与显式记名的既有缺口物品（骨粉与三种马铃薯类物品）
- **AND** 任何物品离开闭包或清单过期的变化 MUST 让判定失败，集合变化 MUST 由人显式确认
