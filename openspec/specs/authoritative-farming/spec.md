...
- **GIVEN** active Ready 范围内一块湿耕地，且湿度待办没有旧积压
...
- **GIVEN** 湿度待办中存在超过 `65,536` 个不在 active Ready 范围内的候选
- **WHEN** 系统推进一个湿度阶段
- **THEN** 该阶段 MUST 恰好检查统一调度器确定性全序下的前 `65,536` 个候选并保留其余待办
- **AND** 该阶段的湿度方块读取次数 MUST 为 `0`
- **AND** 后续阶段 MUST 按同一全序最终排空剩余候选
