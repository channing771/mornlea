...
图形客户端的背包/合成、熔炉与箱子打开态已统一为原创程序化容器像素界面：固定 HUD atlas 的凹槽、三种标题、火焰与箭头 cell 分别供三类 overlay 复用，每个 overlay 只追加一个零 glyph 标题 quad，面板仅向上扩 20px header，栏位与命中几何保持原值；熔炉复用原有进度 bar quad，以火焰自下向上、箭头自左向右裁剪表达权威进度。普通背包、熔炉、箱子的统一栏位仍分别为 36、39、63 格，十条固定配方和两次点击整堆权威移动语义不变；最大打开态为 266 quad，仍装入 267 quad、700 glyph、13312-byte glyph offset 与 46912-byte 固定上传容量。无窗口 capture 现有 18 个正式场景，`main-menu` 插在 `water-surface-slope` 之后、`far-horizon` 之前（`far-horizon` 仍倒数第二、`water-underwater` 仍最后），`chest-container`、`furnace-container` 紧随 `inventory-crafting`。

图形客户端已交付基于 egui 0.35 线（`wgpu` 29 同线、不引入 `egui-winit`、关闭 `default_fonts`）的工具型 UI 基础与主菜单：交互式客户端启动停留在主菜单（标题「Mornlea」、中心纵排按钮列（进入游戏/多人游戏(禁用)/设置(禁用)/退出游戏）、底部版本行与红色错误行），打开世界存档、启动本地权威服务端、登录与远环播种全部延迟到点击「进入游戏」之后；`-connect`、benchmark 与 capture 路径跳过菜单；菜单期间光标不捕获、游戏输入不生效；装配失败在菜单内显示错误行且可重试或退出。菜单字体复用已授权（OFL-1.1、含 provenance）的 Noto Sans CJK SC 经 client ABI 一次性上传，不新增二进制资产；egui 以一条附加 render pass（排在 HUD/debug 之后）挂进既有渲染器，帧不携带 UI 段时完全零参与；既有程序化 HUD、容器 overlay、debug 面板与聊天呈现不迁移。该能力演进自选型文档 `docs/superpowers/specs/2026-08-23-egui-tool-ui-selection-design.md`（工具型 UI 选型：egui，弃 iced/Slint/RAUI/bevy_ui/Go GUI/webview）。benchmark scenario 保持 v19、协议 v26、engine ABI v6、区块 schema v9、世界 metadata v2、玩家 schema v7、`companions.ai` schema v4 与配置格式不变；既有场景 golden 逐字节不变（仅新增 `main-menu.png`）。
