...
服务端权威玩家近战已上线：v25 将既有 `PlayerInput.Mining` 定义为持续 primary action，在同一 tick 对 active 同维玩家按最长 3 格、最近命中和 `SessionID` 平局裁决，固体方块遮挡而流体穿透；有效命中经既有伤害入口造成 2 点伤害并对目标冷却 10 tick，命中只抑制该发起者当 tick 的采掘。协议字段、packet ID、play 载荷长度、玩家/区块/世界/伙伴存档 schema、engine/client ABI 与 benchmark scenario v19 均保持既有契约。
