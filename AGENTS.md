...
图形客户端的背包/合成、熔炉与箱子打开态已统一为原创程序化容器像素界面：固定 HUD atlas 的凹槽、三种标题、火焰与箭头 cell 分别供三类 overlay 复用，每个 overlay 只追加一个零 glyph 标题 quad，面板仅向上扩 20px header，栏位与命中几何保持原值；熔炉复用原有进度 bar quad，以火焰自下向上、箭头自左向右裁剪表达权威进度。普通背包、熔炉、箱子的统一栏位仍分别为 36、39、63 格，十条固定配方和两次点击整堆权威移动语义不变；最大打开态为 266 quad，仍装入 267 quad、700 glyph、13312-byte glyph offset 与 46912-byte 固定上传容量。无窗口 capture 现有 17 个正式场景，`chest-container`、`furnace-container` 紧随 `inventory-crafting`，末三项保持 `water-surface-slope`、`far-horizon`、`water-underwater`。
