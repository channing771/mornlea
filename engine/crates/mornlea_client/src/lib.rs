...
//! (版本见 `mornlea_client_abi_version`,当前 v8,与 `mornlea_engine` ABI
//! 互不耦合)供 Go `internal/client` 调用。client ABI v8 起增加 egui 菜单的两条
//! 出口(字体上传 `render_upload_ui_font`、点击回读 `render_drain_ui_events`)
//! 与帧 TLV tag 9(egui 菜单段)。控制权保持在 Go 主线程:每帧一次
