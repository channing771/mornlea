...
> 用途：从规划表认领任务并按 `docs/development-process.md` 闭环开发，开发完成后**自动收尾**（门禁 → 归档 → 基线同步 → 回填标记 → 合入）。每个任务应派发一个全新实现者会话；本卡同时供常驻实现者与子代理实现者使用。
...
- **接力循环**：`AGENT_LOOP=1` 启动后，每个实现者完成（自动收尾后）即 `relay.sh` 接力启动下一个实现者认领下一行，直到规划表无「就绪」任务自动终结。
...
0. **AGENT_LOOP=1（接力模式）**：守卫登记与防重入已由 `run-agent.sh` 统一完成——启动时以**真实会话 pid** 写入本链守卫（`WORKER_ID` 未设置 → `~/.mornlea/loop.guard`；设置 → `~/.mornlea/loop.guard.$WORKER_ID`），本链守卫已有**存活** pid 则直接退出。**不要再手动 `echo $$`**（旧写法写入的是临时 bash 工具 shell 的 pid，命令一返回进程即死，"存活 pid 检查"永不命中，防重入形同虚设）。不同 WORKER_ID 的链互不排斥，但只有无版本影响且文件集合确实不交叠的任务可并行认领。
1. 读 `docs/feature-backlog.md`：只选一行 `就绪` 任务；`排队` 与 `设计候选` 即使依赖满足也不得认领，同一时间只认领一行。
...
4. 从 `main` 创建 isolation worktree/分支；确认工作区干净。
...
6. 收到批准后把 backlog 状态改为 `开发中`，追加对应【状态变更】评论并刷新 Discussion 正文；未获批准时保持 `已认领`。
...
全部 Task 与整分支终审完成后，先把本行改为 `待集成`，追加对应【状态变更】评论并刷新 Discussion 正文，再执行以下收尾；本行自行完成版本、golden、基线与归档。
...
□ docs/feature-backlog.md 该行 → 已完成（认领人保留履历）
...
□ 接力循环（AGENT_LOOP=1 时）：rm -f ~/.mornlea/loop.guard${WORKER_ID:+.${WORKER_ID}} && scripts/agents/relay.sh（清**本链**守卫；relay 按同一链身份接力，工具继承 AGENT_TOOL）
   —— 无就绪任务时 relay.sh 自动终结循环；手动单次运行（AGENT_LOOP=0）跳过
...
```text
【状态变更】<ID> <功能名> → <状态：就绪|已认领|开发中|待集成|排队|设计候选|已完成|已取消>
...
- PR 合并前确认整分支终审、`scripts/agents/gates.sh` 门禁子集与单独的 `make rust-check` 均通过，且无未推送功能分支依赖本行产出；只有无版本影响且文件集合确实不交叠的任务可并行，多条流水线并行时逐条监听，任一失败都先修复再合并。
