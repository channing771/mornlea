...
1. `docs/feature-backlog.md` —— 当前规划与认领状态（单一真相源）。状态只允许 `就绪|已认领|开发中|待集成|排队|设计候选|已完成|已取消`；B–F 组为七列表（含「版本与契约影响」），A 组为六列（无版本列）。
...
### 3. 拆解扩展
- 每个经确认的新缺口拆成**一条**规划行；可独立闭环的行对应一个可独立评审的 OpenSpec change 并默认进入 `排队`，umbrella、推测性或尚无独立闭环的功能进入 `设计候选`。新增行 ID 按组连续编号（上一行尾号 +1，不跳号）。
- 每行必须齐备：ID / 功能名 / 简述 / 版本与契约影响 / 状态（`排队` 或 `设计候选`）/ 认领人（`—`）/ 来源（具体文档名 + 条目号或 §）。
...
- 依赖关系写进「来源与备注」（只写行 ID，不写日期）；**不排时间**，不写日期与估时。
...
2. **表格结构**：B..F 组 7 列（ID|功能|简述|版本与契约影响|状态|认领人|来源与备注），A 组 6 列（无版本列）；新增行字段与表头一致；`版本与契约影响` 列只写方向性结论（如「协议升版」），绝对版本号按合入时序重排。
3. **状态词八种**：`就绪|已认领|开发中|待集成|排队|设计候选|已完成|已取消`；状态列外不写过程性状态（如「待确认」写进备注）。
4. **晋升归 planner/控制会话**：新缺口默认 `排队`，umbrella、推测性或尚无独立闭环的功能为 `设计候选`；只在前序已完成、版本槽空闲、worktree 证据一致时晋升串行队首，且不得同时晋升两个会推进稳定编号、协议/schema、ABI 或 golden 的核心玩法行。
5. **ID 连续**：按组上一行尾号 +1；不跳号、不复用。
6. **来源必填且入库**：每行来源可回溯到已提交文档（design 遗留清单 §/proposal 非目标/批次设计章节）；未入库引用只记录不代交。
7. **评论样式**：状态变化一律单条结构化评论（模板见 `docs/agents/implementer.md`）；正文刷新 = 八组状态的脚本镜像，只有 `就绪` 是绿色可领取组；评论 = 变更记录；不重复堆叠旧状态。
...
- Go/Rust 标识符用反引号包裹。
- 状态只允许：`就绪|已认领|开发中|待集成|排队|设计候选|已完成|已取消`。
