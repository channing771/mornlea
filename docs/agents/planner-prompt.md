...
1. 读 `docs/feature-backlog.md` —— 当前规划的**单一真相源**：状态只允许 `就绪|已认领|开发中|待集成|排队|设计候选|已完成|已取消`，同时核对认领人列与 ID 分配（A-xx……F-xx，注意各组已用的最大编号）。B–F 组表是七列结构（含「版本与契约影响」）；A 组是六列（无版本列）。
...
- 每行必须齐备 7 个字段：`ID` / `功能` / `简述` / `版本与契约影响` / `状态`（`排队` 或 `设计候选`）/ `认领人`（`—`）/ `来源与备注`（具体文档名 + 条目号或 §，可加依赖行 ID、互斥说明）。
...
2. **表格结构**：B..F 组 7 列（ID|功能|简述|版本与契约影响|状态|认领人|来源与备注）；A 组 6 列（无版本列）；新增行字段与表头一致；版本列只写方向性结论。
3. **状态词八种**：`就绪|已认领|开发中|待集成|排队|设计候选|已完成|已取消`；「待确认」等过程态写进备注而非状态列。
...
- 术语沿仓库既有中文术语（权威 / 原子 / 有界 / 确定性…），Go/Rust 标识符用反引号包裹。
