...
- 失败语义分两层：单帧捕获失败或单帧交付超时只终止采样，已收到的帧连同终止原因（`manifest.error`）仍以 200 zip 交付——局部证据好于全盘丢弃；整段录制超过总时长上限（名义时长 + 30s 固定余量 + 每帧 2.5s 预算 × 总帧数；预算按实测最慢路径——GIF 量化约 2.2s/帧——放大）仍完不成（帧循环不可用）才以 503 放弃整次录制，错误 JSON 回显公式三段数值。
