...
`cmd/mornlea/app`、`cmd/mornlea/capture`、`cmd/mornlea/benchmark` 与 `internal/server` 中的重型测试（每个超过数秒的离屏 renderer 场景、golden 场景抓帧、长 tick 集成/容量测试）在 `testing.Short()` 时跳过：
