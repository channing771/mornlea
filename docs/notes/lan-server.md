...
版本矩阵：协议 v32、玩家 schema v8、区块 schema v9、世界 metadata v3、`companions.ai` schema v4、`hostile_mobs` schema v1、engine ABI v9、client ABI v14、benchmark scenario v20。升级前必须正常关服并备份完整世界目录；不要让旧程序直接打开已升级目录后继续写入。异常退出时各文件分别原子更新，但文件之间没有跨文件事务。
