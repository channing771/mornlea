...
- 输入:一个**邻域盒**——以本次被扫区块为中心的 18×256×18 体素盒(中心区块全量 + 四侧 1 格裙边;裙边取自周围 8 个区块的贴面列,不可达读 `BarrierID`);区段压缩表示:均匀段 = 标记 + ID(4B),非均匀段展开 4096×2B。附小元数据表:被扫区块与周围 8 个裙边贡献区块(共 ≤9)各 24 区段的 uniform 位与均匀 ID(≈650B),供区段级不动点捷径判定。
...
- **重扫组装成本**:每 (区块, 平面) 调用现场组装 ≤~170KB;均匀段占绝对多数时实际远小,且仅发生在该区块的重扫窗口内;若实测超预期,压缩编码已预留 Palette 直通路径。
