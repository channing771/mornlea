...
| B-17 | 门 | 方块 + 交互开合，原创模型 | 方块编号追加 + 开合状态编码 | 已认领 | opencode-implementer @ feat/B-17-door | 批次设计非目标；B-17→B-30→B-11→B-24→B-23→B-35→B-36→B-37 串行链队首；2026-08-27 控制会话晋升排队→已认领（队首 B-17 可独立推进，不占协议/schema 预分配版本号）；独占文件集：`internal/core`（门方块/物品编号段 append-only）、`internal/assets`（门模型/纹理/配方登记）、`internal/sim`（放置/开合/支撑与碰撞）、`internal/mesh`/`engine`（门几何）、`openspec/changes/authoritative-door` |
