...
| B-06 | 干耕地退回泥土 | 需第三个耕地编号或附加状态字节 | 方块编号追加或状态编码 | 已认领 | opencode-implementer @ feat/B-06-dry-farmland-revert | farming 遗留 5；2026-08-27 控制会话晋升排队→就绪并认领（bounded：干耕地在持续干燥且无作物时退回泥土，复用既有随机刻与湿度判定），独占文件集：`internal/sim/farmland_revert.go`及同包测试、`internal/core` 耕地/泥土编号段、`openspec/changes/dry-farmland-revert/`；刻意不触碰 `internal/fluid`、`internal/sim/crop.go` 生长路径、`engine_step.go`/`tunables.go`（A-04/B-09）与 `internal/network`/`internal/storage` |
