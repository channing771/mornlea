...
| C-08 | 伙伴空闲随机聊天 | 非任务触发台词 | 复用既有 `CompanionSpeech`，无 wire 变更 | 就绪 | — | §16；2026-08-27 控制会话批准最小闭环：只有最近任务发令者仍在线且位于伙伴水平 16 格内时，完全空闲的伙伴才按每伙伴确定性的 60–120 秒间隔触发 Dialogue；复用既有模型并发槽、单在途纪律与 `CompanionSpeech` 广播，不改任务/FIFO/摘要/持久化、协议/schema/ABI/scenario 或 golden。与 A-01/E-12 在途独占文件集零交集。原认领履历 `opencode-implementer @ feat/C-08-companion-idle-dialogue` 未发现对应 worktree、branch 或开放 PR，本次经控制会话重新晋升 |
