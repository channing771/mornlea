...
| A-07 | 版本基线与视觉基线 | 协议 v27 / schema v8 / metadata v3 / `hostile_mobs` v1 / engine ABI v7 / client ABI v10 / benchmark v20 迁移 `19:20`；生成新 5 场景 golden；同步 `AGENTS.md`+`CLAUDE.md`+`progress.md` | 未认领 | — | 依赖 A-06。client ABI 按已占用的 v8 主菜单、v9 设置页重排为 v10；场景总数按当前 19 个加新增 5 个计算为 24，批次历史计划中的 22 未含 `main-menu`/`settings-menu`。**占用 v10 意味着 D 组（egui 工具型 UI 行）在 A-07 合流前全部被锁**——D-02 暂停菜单 / D-03 调试面板 egui 化均需新 UI layout（v3+）跨 client ABI；2026-08-26 一次认领 D-03 时即因此退回，A-07 领受时应在备注前记此约束 |
...
| D-03 | 调试面板 egui 化 | 既有程序化 debug 面板迁入 egui 或并存；性能影响需评审 | 无 wire（呈现层） | 已认领 | opencode-implementer @ feat/D-03-debug-panel-egui | 2026-08-26 认领；egui 实施路径。范围冻结：把既有程序化 debug 面板迁入 egui（或并存），呈现层改动，无 wire/schema/ABI/scenario。独占文件集：`internal/render/debug_panel.go`、`cmd/mornlea/debug_panel.go` 及相关定点测试；控制会话批准与 A-01/A-04 的三处最小受控接线重叠（B-14/B-31 先例同形）：`cmd/mornlea/app.go` 仅字段行、`app_startup.go` 仅构造行、`app_frame.go` 仅调用点。golden：`debug-panel` capture 场景将按实现再生（不触碰其它场景），最终 `make visual` 人工验收 |
