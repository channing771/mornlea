...
| D-03 | 调试面板 egui 化 | 既有程序化 debug 面板迁入 egui 或并存；性能影响需评审 | 待集成 | opencode-implementer @ feat/D-03-debug-panel-egui | egui 实施路径；2026-08-26 曾由 `opencode-implementer @ feat/D-03-debug-panel-egui` 认领，因原 A-07 client ABI v10 预占冲突退回；A-07 取消后于 2026-08-27 经控制会话授权重新设计（完全迁移删旧程序化渲染路径、保留行选中+值编辑+F3 全捕获）并重新认领。实现与双评审+整分支终审完成，PR #98 开（等待合入）。独占文件集（实际触碰）：`engine/crates/mornlea_client/src/ui.rs`（`UI_DEBUG_LAYOUT_VERSION=3` 段解码与 egui 面板控件）+ `ui/debug_abi_tests.rs`/`debug_render_tests.rs`/`test_support.rs`/`menu_render_tests.rs`/`settings_render_tests.rs`、`cmd/mornlea/debug_panel.go`（layout v3 段编码与事件回传）、`internal/render/debug_panel.go` 删程序化绘制（保留 `PanelRow`/`PanelReadout`）、`internal/render/frame_streams.go`、`cmd/mornlea/app_frame.go`/`app_startup.go`/`interactive.go` 接线、OpenSpec change `egui-debug-panel` 与 `debug-panel.png` golden 再生（用户人工确认）；无 wire 变更，client ABI v9，不触碰 A-01/E-12 在途文件集 |
...
| E-12 | M5E 再递延字面同源化 | `capture_scene.go` 与 `capture_ai_companion_test.go` 的 `[32]network.ChatEvent` 字面；ChatCommand 编解码 1024 字面与错误文案 | 无（字面同源化） | 开发中 | zcode-implementer @ chore/E-12-literal-single-source（2026-08-27 接手续做） | m5e 归档 proposal「延期与放弃」递延 4、5；独占文件集：`cmd/mornlea/capture_scene.go`、`cmd/mornlea/capture_ai_companion_test.go`、`internal/network/codec_client.go` 及相关定点测试 + OpenSpec change `m5e-literal-single-source`；2026-08-25 原认领（chore/E-12-companion-limits）无推送记录，随机器迁移重置重认领；2026-08-25 本行由 claude-implementer 认领，分支 chore/E-12-literal-single-source；worktree 有未提交实现，允许原认领者收尾；2026-08-27 控制会话裁决转移给 zcode-implementer 续做：原认领者进程已不存在且 worktree 自 2026-08-25 晚起长期零活动（机器迁移失联先例），沿既有分支与已批准设计（ledger 已录飞书 approve）执行，范围与独占文件集不变 |
