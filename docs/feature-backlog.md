...
| D-04 | 合成面板分页/滚动 | 配方行数增长后按窗口高度自适应（当前 10 行，矮窗口整体缩小） | 无 wire；HUD 布局（capture 校验） | 已认领 | ox-alpha-implementer @ feat/D-04-crafting-panel-paging | farming 遗留 20；2026-08-26 认领。范围冻结（bounded）：纯呈现层——合成面板在可用高度不足时按行分页（或滚动，brainstorming 裁决）替代整体等比缩小，参考分辨率下呈现逐字节不变、既有 capture golden 不动。独占文件集：`internal/render/hud/layout.go`、`renderer.go` 中合成面板布局函数与同包新增测试文件；刻意不触碰 `container.go` overlay 命中几何、`cmd/mornlea` 全部装配文件、协议/schema/ABI/scenario 与 `AGENTS.md`/`CLAUDE.md` |
