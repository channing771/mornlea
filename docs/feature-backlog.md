...
| A-07 | 版本基线与视觉基线 | 原批次统一基线职责已拆回各功能行 | 已取消 | — | 原来源：第一夜批次版本与视觉基线任务；版本、golden 和基线职责回收到各功能行 |
...
| C-08 | 伙伴空闲随机聊天 | 非任务触发台词 | 复用既有 `CompanionSpeech`，无 wire 变更 | 设计候选 | — | §16；原认领履历 `opencode-implementer @ feat/C-08-companion-idle-dialogue`，范围曾冻结为伙伴空闲 Dialogue 的确定性触发、既有模型并发纪律复用与广播接线；未发现对应 worktree、branch 或开放 PR，若出现未同步证据先恢复证据再裁决 |
...
| D-03 | 调试面板 egui 化 | 既有程序化 debug 面板迁入 egui 或并存；性能影响需评审 | 无 wire（呈现层） | 设计候选 | — | egui 实施路径 |
| D-04 | 合成面板分页/滚动 | 配方行数增长后按窗口高度自适应（当前 10 行，矮窗口整体缩小） | 无 wire；HUD 布局（capture 校验） | 已取消 | — | farming 遗留 20 |
