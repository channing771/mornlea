...
| D-12 | 菜单层迁移 WebView（Vite+TS+React）与世界全景背景 | 主菜单/设置/暂停/F3 迁进程内 WKWebView + Vite/TS/React，egui 完全退役；主菜单世界全景背景（worldgen 直供，不装配存档/服务端）；桥 schema 三端钉值；菜单 chrome 退出像素 golden 改组件断言 | client ABI v11→v12；协议/schema/存档/benchmark scenario 不变；golden 两张 UI 图内容换全景底图 | 已认领 | zcode-control @ feat/client-webview-ui | 2026-08-29 用户裁决：进程内 WebView、Vite+TS+React、F3 同迁 egui 退役、世界全景背景；change `client-webview-ui`（egui-tool-ui capability 退役由 webview-menu-ui 接替）；D-10 当日实测菜单「太丑」为直接动因 |
