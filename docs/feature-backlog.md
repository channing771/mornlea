...
- **同批并行必须共享契约**：参照第一夜批次先例——第一步在功能分支共同基线上冻结「追加编号 / 协议消息 / 有限模型 tag」的 append-only 共享契约提交，各功能分支从该 SHA 创建；根 `AGENTS.md` 版本矩阵、相关局部指南、capture golden、benchmark scenario 与版本基线由**集成任务独占**，功能分支不得触碰（避免并行覆盖 PNG 与文档冲突）。
...
| A-07 | 版本基线与视觉基线 | 协议 v27 / schema v8 / metadata v3 / `hostile_mobs` v1 / engine ABI v7 / client ABI v10 / benchmark v20 迁移 `19:20`；生成新 5 场景 golden；同步根 `AGENTS.md` 版本矩阵、相关局部指南与 `progress.md` | 未认领 | — | 依赖 A-06。client ABI 按已占用的 v8 主菜单、v9 设置页重排为 v10；场景总数按当前 19 个加新增 5 个计算为 24，批次历史计划中的 22 未含 `main-menu`/`settings-menu` |
