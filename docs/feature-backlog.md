...
| D-03 | 调试面板 egui 化 | 既有程序化 debug 面板迁入 egui 或并存；性能影响需评审 | 无 wire（呈现层） | 已认领 | opencode-implementer @ feat/D-03-debug-panel-egui | egui 实施路径；2026-08-26 曾由 `opencode-implementer @ feat/D-03-debug-panel-egui` 认领，因原 A-07 client ABI v10 预占冲突退回；A-07 取消后于 2026-08-27 经控制会话授权重新设计（完全迁移删旧程序化渲染路径、保留行选中+值编辑+F3 全捕获）并重新认领。独占文件集：`engine/crates/mornlea_client/src/ui.rs`（新增 `UI_DEBUG_LAYOUT_VERSION=3` 定宽行记录段解码与 egui 面板控件）、`cmd/mornlea/debug_panel.go`（layout v3 段编码改造，保留 `remote()`/`panelModeLabel()`/`applyPanelChange()`）、`internal/render/debug_panel.go`（删程序化绘制，保留 `PanelRow`/`PanelReadout` 类型与 `config.Fields()`/`rows()` 数据源）、`internal/client/window.go`（编辑键枚举）、`cmd/mornlea/interactive.go`（F3 切换与键盘边沿状态）、及对应测试与 OpenSpec change；无 wire 变更（客户端内呈现），不升 client ABI（ln v9 内扩 layout 常量，D-01 先例），不触碰 A-01/Z-12 在途独占文件集 |
