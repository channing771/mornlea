...
| B-04 | 草丛与除草掉种子 | 植物几何第二消费者；草丛掉落种子替代初始材料包供给 | 植物区间编号追加 | 已认领 | codex-control @ refactor/sim-ownership-convergence | farming 遗留 6；依赖 A-05；同一行取消固定 64 种子赠送；范围冻结：单格短草、自然生成、1/8 种子掉落、仅取消新玩家固定 64 种子赠送；独占文件集：OpenSpec、`internal/core`、`internal/assets`、`internal/mesh`、`internal/physics`、`internal/worldgen`、`internal/nativeabi`、`internal/lod`、`internal/sim/entity` 与 `internal/sim/realm`、`internal/server` persistence 与 e2e、benchmark/perfcheck/capture 相关文件及版本基线文档；排除其他用户改动 |
