...
| B-30 | 冲刺（疾跑） | 移动输入位与速度提升 | `PlayerInput` 追加输入位 → 协议升版 | 已认领 | opencode-implementer @ feat/B-30-sprint | hunger 遗留 6（「动作不存在」）；B-13 冲刺疲劳依赖本行；依赖 B-17 已满足（B-17 已完成 2026-08-27，串行链队首晋升）；2026-08-27 控制会话经 brainstorming（bounded）晋升排队→已认领，方案 A 最小闭环：`PlayerInput.Sprinting`+`ProtocolVersion 27→28`、地面+饥饿≥6+向前+非潜行/浸没门控、1.3x（`SprintSpeedMultiplierMilli=1300` tunable）水平加速、疲劳表新增 sprint 行走 `applyExhaustion` 循环、无 FOV/HUD/音效；独占文件集：`internal/network`（`PlayerInput`/`message_command`/`packet`/`codec_client`）、`internal/physics`+`engine/crates/mornlea_engine/src/step.rs`（`StepInput` 头版本与 Rust 加速）、`internal/sim/{tunables,hunger,player}`、`openspec/changes/sprint` |
...
| D-02 | 暂停菜单 | 单机暂停最小闭环：Esc 开合暂停覆盖层、权威 tick 冻结、退回主菜单 | 无 wire 变更（权威暂停门 + 呈现层） | 已认领 | zcode-implementer @ feat/D-02-pause-menu | egui 实施路径；2026-08-27 控制会话晋升排队→就绪并经用户批准 bounded 边界后认领：`internal/server` 加原子暂停门（`Pause()`/`Resume()`，`RunTicks` 调度前检查），TCP 远程会话不宣称暂停（服务端照常 tick，菜单注明）；菜单条目仅「返回游戏 / 退回主菜单」（退回复用既有 closeClientSession 拆链路径），设置入口列为顺延项（D-01 设置页现假设世界未装配，游戏中途进设置属新语义）。capture 场景与 golden 零变化；协议 v27 与各 schema、engine/client ABI、benchmark scenario 均不变。独占文件集：`cmd/mornlea/app_menu.go`、`interactive.go`（Esc 栈一处）、新增 `app_pause*.go` 及测试、宿主装配微调、`internal/server/server.go` 暂停门及同包测试、`engine/crates/mornlea_client/src/ui.rs` 或 `ui/` 新暂停页及测试、OpenSpec change 目录；与在途 A-02（core 编号段/assets/sim/mesh/engine crate/nativeabi/capture_scene*）和 B-17（core 门方块编号段/assets/sim 放置/mesh-engine 几何）文件集零交集 |
