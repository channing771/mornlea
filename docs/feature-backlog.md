...
|---|---|---|---|---|---|
| A-01 | 权威格子工作台 | 背包 2×2 + 工作台 3×3 形状配方、权威产物取出、关闭/断线装回不变量、七条新配方 | 开发中 | zcode-implementer @ feat/A-01-authoritative-grid-crafting | 原分支头 `a657c1cb`（实现与整功能评审完成）未推送远端已丢失；当前分支头 `8c2ea638`。独占文件集：`internal/core`（`recipe.go` 网格匹配与 recipe 1..18 形状、`inventory.go`、`item.go` 及测试）、`internal/assets/blocks.go`（工作台方块与纹理及测试）、`internal/sim`（新建 `crafting.go` 与 `player.go`/`command.go`/`engine_step.go`/`container.go`/`drop.go`/`death.go`/`persistence.go` 及测试）、`internal/network` crafting 消息（`message_inventory.go`/`codec_client.go`/`codec_server.go`/`packet.go` 及测试）、`internal/server`（`session_ingress.go`/`publication.go` 及 parity 测试）、`internal/client/inventory.go` 及镜像测试、`internal/render/hud/container.go` 及测试、`cmd/mornlea`（`app*.go` 合成 UI 与 `capture_scene*` 场景构造、不写 golden）及 OpenSpec change `authoritative-grid-crafting`；因整堆移动令多格同材配方不可正常制作而重开最小单件拆分；本行自行完成协议/golden/归档/合入 |
| A-02 | 可放置火把 | 落地 + 四向墙面五形态、支撑约束与移除反应、发光等级 14、mesh model tag 与窄柱几何 | 排队 | — | 原认领履历 `ox-alpha-implementer @ feat/A-02-placeable-torches`；原分支头 `b2115a64`（实现与整功能评审完成）未推送远端已丢失。原独占文件集（仅作履历，不再占用）：`internal/core` 方块属性/物品/配方登记、`internal/assets`、`internal/sim` 火把放置与支撑、`internal/world/chunk.go`、`internal/mesh`、`internal/nativeabi`、`engine/crates/mornlea_engine`、`openspec/changes/placeable-torches`、`cmd/mornlea/capture_scene*`；worktree 有未提交 `core/assets/physics` 改动，A-01 合入前不得清理或 rebase |
...
| B-03 | 骨粉 | 新物品 + 「立即推进 N 阶段」动作，走翻地同形命令路径 | 物品编号追加；命令段可能追加 | 排队 | — | farming 遗留 3 |
...
| B-06 | 干耕地退回泥土 | 需第三个耕地编号或附加状态字节 | 方块编号追加或状态编码 | 开发中 | opencode-implementer @ feat/B-06-dry-farmland-revert | farming 遗留 5；2026-08-27 控制会话晋升排队→就绪并认领（bounded：干耕地在持续干燥且无作物时退回泥土，复用既有随机刻与湿度判定），独占文件集：`internal/sim/farmland_revert.go`及同包测试、`internal/core` 耕地/泥土编号段、`openspec/changes/dry-farmland-revert/`；刻意不触碰 `internal/fluid`、`internal/sim/crop.go` 生长路径、`engine_step.go`/`tunables.go`（A-04/B-09）与 `internal/network`/`internal/storage`；2026-08-27 brainstorming bounded 批准：干+无作物才退，随机tick概率30%，复用crop抽样有界性 |
...
| B-12 | 饱和抖动提示 | `PlayerState` 追加 `SaturationZero` 一位 | 协议升版（`PlayerState` 追加字段） | 设计候选 | — | hunger 遗留 3 |
...
| B-17 | 门 | 方块 + 交互开合，原创模型 | 方块编号追加 + 开合状态编码 | 排队 | — | 批次设计非目标；B-17→B-30→B-11→B-24→B-23→B-35→B-36→B-37 串行链队首 |
...
| B-30 | 冲刺（疾跑） | 移动输入位与速度提升 | `PlayerInput` 追加输入位 → 协议升版 | 排队 | — | hunger 遗留 6（「动作不存在」）；B-13 冲刺疲劳依赖本行；依赖 B-17 |
...
| C-08 | 伙伴空闲随机聊天 | 非任务触发台词 | 复用既有 `CompanionSpeech`，无 wire 变更 | 已认领 | opencode-implementer @ feat/C-08-companion-idle-dialogue | §16；2026-08-27 控制会话批准最小闭环：只有最近任务发令者仍在线且位于伙伴水平 16 格内时，完全空闲的伙伴才按每伙伴确定性的 60–120 秒间隔触发 Dialogue；复用既有模型并发槽、单在途纪律与 `CompanionSpeech` 广播，不改任务/FIFO/摘要/持久化、协议/schema/ABI/scenario 或 golden。与 A-01/E-12 在途独占文件集零交集。原认领履历未发现对应 worktree、branch 或开放 PR，本次由 `opencode-implementer @ feat/C-08-companion-idle-dialogue` 重新认领。独占文件集：`internal/companion/dialogue_nodes.go`、`dialogue_client.go` 及对应定点测试，`internal/server/companion_manager.go`、`companion_dialogue.go`、新建 `companion_idle_dialogue.go` 及对应测试，OpenSpec change `companion-idle-dialogue` 与本行设计/计划/ledger 文档；刻意不触碰 Planner、任务/FIFO 实现、`internal/network`、`internal/storage`、协议/schema/ABI/scenario、capture/golden 与长期基线文档 |
...
| D-02 | 暂停菜单 | 暂停/继续/退出；先设计单人权威模拟的暂停语义 | 视暂停语义裁决 | 排队 | — | egui 实施路径 |
| D-03 | 调试面板 egui 化 | 既有程序化 debug 面板迁入 egui 或并存；性能影响需评审 | 无 wire（呈现层） | 设计候选 | — | egui 实施路径；2026-08-26 曾由 `opencode-implementer @ feat/D-03-debug-panel-egui` 认领，因原 A-07 client ABI v10 预占冲突退回；A-07 取消后仍须按当前队列重新设计和晋升 |
