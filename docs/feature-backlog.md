...
| E-09 | 作物×锄头耐久豁免 | 对齐 MC：手持锄头收获不扣耐久，`completeMining` 加豁免表并配测试 | 无（豁免表 + 测试） | 已认领 | claude-implementer @ fix/E-09-hoe-harvest-durability | farming 遗留 16；独占文件集：`internal/sim/mining.go` 及 `internal/sim/` 相关测试 + OpenSpec change `fix-hoe-harvest-durability` |
