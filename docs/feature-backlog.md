...
| E-10 | `findSpawnInColumn` 读落脚盒顶面 | 出生点与 support/safe 存档点三处口径同步（耕地 1/16 空隙） | 无（出生点口径） | 开发中 | codex-implementer @ fix/E-10-spawn-support-top | farming 遗留 14；独占文件集：`internal/sim/spawn.go`、出生/恢复相关测试 + OpenSpec change `fix-spawn-support-top`；`E-10-approval1` 已批准 bounded 短设计 |
