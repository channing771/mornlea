...
| E-09 | 作物×锄头耐久豁免 | 对齐 MC：手持锄头收获不扣耐久，`completeMining` 加豁免表并配测试 | 无（豁免表 + 测试） | 开发中 | codex2-implementer @ fix/E-09-hoe-harvest-durability | farming 遗留 16；独占文件集：`internal/sim/mining.go` 及 `internal/sim/` 相关测试 + OpenSpec change `fix-hoe-harvest-durability`；Task 1 实现与 SPEC/QUALITY 双评审完成（`fbf5d82c`），进入门禁与收尾 |
| E-10 | `findSpawnInColumn` 读落脚盒顶面 | 出生点与 support/safe 存档点三处口径同步（耕地 1/16 空隙） | 无（出生点口径） | 已认领 | codex-implementer @ fix/E-10-spawn-support-top | farming 遗留 14；独占文件集：`internal/sim/spawn.go`、出生/恢复相关测试 + OpenSpec change `fix-spawn-support-top` |
