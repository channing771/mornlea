...
三大家族按“可观察对象 × 时间维度”路由，文件名与数量以各注册表为准：
...
- 第三类过程 GIF（`motion/`）：跨 tick 状态迁移的全流程 GIF，按 tick 步进抓帧，覆盖触发前、结算、收敛全过程，不得只截片段。它们是有界的人工审查证据，只验呈现、不进任何自动像素比对门禁；其中被动牛 4 剧本随 `make visual-update` 由 `RunCapture` 内的 `RunPassiveDeathGIFs` 生成，motion 演示 7 个经 `--motion-demo` 显式生成。
