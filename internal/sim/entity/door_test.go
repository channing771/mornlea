...
func TestDoorPlaceAndToggle(t *testing.T) {
	...
	}
	{
		...
		if !handleInteractDoor(engine.engineContext, core.Overworld, lower, engine.newMutation()) {
		...
		if !handleInteractDoor(engine.engineContext, core.Overworld, upper, engine.newMutation()) {
...
func TestDoorInteractViaRaycast(t *testing.T) {
	...
	pending := engine.newMutation()
	if !handleInteractDoor(engine.engineContext, core.Overworld, upper, pending) {
...
func TestDoorPhysicsFluidDivergence(t *testing.T) {
	...
	pending := engine.newMutation()
	handleInteractDoor(engine.engineContext, core.Overworld, lower, pending)
	engine.finishChanges(pending, &TickResult{})
	if solid, _ := sampler(lower); solid {
