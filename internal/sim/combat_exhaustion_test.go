...
// TestMeleeExhaustionThresholdCrossSettlesSaturationThenHunger 覆盖 Scenario
...
func TestMeleeMissPathsAccumulateNoExhaustion(t *testing.T) {
	...
	t.Run("受击冷却免疫", func(t *testing.T) {
		...
		attacker := engine.sessions[sessions[0]].player
		target := engine.sessions[sessions[1]].player
		// 预置正冷却：意图收集要求目标 `meleeCooldownTicks` 恰为 0，
		// 冷却窗口内的每一次挥拳都必须在冻结处被丢弃。
		target.meleeCooldownTicks = 20
		attacker.miningHeld = true
