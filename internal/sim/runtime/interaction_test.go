...
func TestEngineMiningValidation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			rejected:  true,
			reason:    runtime.RejectInvalidInput,
...
func TestEnginePlaceValidationAndWhitelist(t *testing.T) {
	t.Run("empty slot", func(t *testing.T) {
		...
		assertRejected(t, engine.Step(), runtime.RejectInvalidBlock)
	})
	...
	t.Run("origin inside solid is occupied", func(t *testing.T) {
		...
		assertRejected(t, engine.Step(), runtime.RejectOccupied)
	})
...
func TestPlayerIntentDoesNotModifyReadyChunkWhenRayEntersUnknownChunk(t *testing.T) {
	tests := []struct {
		...
		{name: "mine", command: runtime.Command{Kind: runtime.CommandPlayerInput, Mining: true}},
		{name: "place", command: runtime.Command{Kind: runtime.CommandPlaceBlock, Slot: 0}, wantReject: true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result := engine.Step()
			if test.wantReject {
				assertRejected(t, result, runtime.RejectChunkNotReady)
...
func TestInteractionReachTunableGatesPlacement(t *testing.T) {
	...
	place := func(t *testing.T, reach float32) runtime.TickResult {
		t.Helper()
	...
	assertRejected(t, place(t, 2), runtime.RejectNoTarget)
}
