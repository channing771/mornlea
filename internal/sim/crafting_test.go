...
// 与工作台生命周期。recipe-click 的过渡拒绝（design.md D6）同样归属本主题。
...
func craftingTakeCommand(session sim.SessionID, sequence uint64) sim.Command {
	return sim.Command{
		Session:  session,
		Sequence: sequence,
...
func currentCraftingGrid(
	...
	engine *sim.Engine,
	session sim.SessionID,
) (sim.CraftingGrid, core.ItemStack) {
	t.Helper()
...
func TestCraftingMoveRejectsPersonalExtendedSlots(t *testing.T) {
	...
	for _, rejection := range result.Rejected {
		if rejection.Reason != sim.RejectInvalidSlot {
...
// oracle，故意不复用被测代码。
func repackOracle(inventory core.Inventory, grid sim.CraftingGrid) bool {
...
// 返回引擎、会话与递增的命令序号指针。
func repackMarginalSetup(t *testing.T) (*sim.Engine, sim.SessionID, *uint64) {
	t.Helper()
	var inventory core.Inventory
...
func gridState(t *testing.T, engine *sim.Engine, session sim.SessionID) sim.CraftingGrid {
	t.Helper()
	grid, _ := currentCraftingGrid(t, engine, session)
	return grid
}
...
func TestCraftingRepackContainerMoveRejectedWhenItWouldBreakInvariant(t *testing.T) {
	...
	*sequence++
	engine.Enqueue(sim.Command{
		Session: session, Sequence: *sequence, Kind: sim.CommandOpenFurnace, Pitch: lookDown,
	})
	...
	*sequence++
	engine.Enqueue(sim.Command{
		Session: session, Sequence: *sequence, Kind: sim.CommandMoveFurnaceStack,
...
func TestWorkbenchOpenSetsSizeThreeWithoutContainerRef(t *testing.T) {
	...
	engine.SetBlockForTest(core.BlockPos{}, core.WorkbenchID)
	engine.Enqueue(sim.Command{
		Session: session, Sequence: 2, Kind: sim.CommandOpenFurnace, Pitch: lookDown,
	})
	result := engine.Step()
	if len(result.Rejected) != 0 {
		t.Fatalf("打开工作台被拒绝: %+v", result.Rejected)
...
func TestWorkbenchMiningTakesFifteenTicksAndDropsOneWorkbench(t *testing.T) {
	...
	if len(final.Rejected) != 0 || len(final.Changes) != 1 ||
		final.Changes[0].Changes[0] != (sim.BlockChange{Position: core.BlockPos{}, Block: core.AirID}) {
...
func TestWorkbenchGridsOfTwoPlayersAreIndependent(t *testing.T) {
	...
	engine.SetBlockForTest(core.BlockPos{}, core.WorkbenchID)
	engine.Enqueue(sim.Command{
		Session: first, Sequence: 2, Kind: sim.CommandOpenFurnace, Pitch: lookDown,
	})
	engine.Enqueue(sim.Command{
		Session: second, Sequence: 2, Kind: sim.CommandOpenFurnace, Pitch: lookDown,
	})
	if result := engine.Step(); len(result.Rejected) != 0 {
...
func TestCraftingCloseCommandRepacksExtendedSlots(t *testing.T) {
	...
	engine.Enqueue(sim.Command{
		Session: session, Sequence: sequence, Kind: sim.CommandCloseFurnace,
	})
	result := engine.Step()
	if len(result.Rejected) != 0 {
