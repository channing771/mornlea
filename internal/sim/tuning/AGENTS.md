...
`internal/sim/tuning` 持有 `Tunables`、默认值、钳制与原子活动快照，是配置与模拟之间的唯一参数源。
...
- 允许：`internal/core`。
- 禁止：依赖 `internal/sim/contract`/`realm`/`entity`/`runtime` 或 `internal/world`/`internal/physics` 等上层状态；禁止反向依赖。
- 方向由 `internal/archcheck` 强制，`tuning` 为叶子，任何对其上层包的依赖均为反向边。
...
- `go test ./internal/sim/tuning -race -count=1`
- 关联：`go test ./internal/config -race -count=1`（配置装配）
