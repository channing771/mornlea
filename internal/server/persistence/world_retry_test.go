...
func TestFlushFrozenFailureReleasesUnsentPendingJobsForLaterRetry(t *testing.T) {
	...
	failed := saveJob{Region: region, Snapshots: first, Attempt: 1}
	running.saveJobs <- failed
	wantErr := errors.New("failed in-flight shutdown save")
	firstFlush := make(chan error, 1)
	go func() { firstFlush <- running.flushFrozen(context.Background()) }()

	deadline := time.Now().Add(waitDeadline)
	for {
		running.mu.Lock()
		pending := engine.PersistenceStats().InFlightChunks == len(keys)
		running.mu.Unlock()
		if pending {
			break
		}
		if time.Now().After(deadline) {
			t.Fatal("flush did not create unsent pending shutdown jobs")
		}
		time.Sleep(integrationPollInterval)
	}
	running.saveCompletions <- saveCompletion{Job: failed, Err: wantErr}
	select {
	case err := <-firstFlush:
		if !errors.Is(err, wantErr) {
			t.Fatalf("first flush error=%v, want identity %v", err, wantErr)
		}
	case <-time.After(waitDeadline):
		t.Fatal("first flush did not return failed completion")
	}
	if got := engine.PersistenceStats(); got.DirtyChunks != len(keys) || got.InFlightChunks != 1 {
		t.Fatalf("unsent pending snapshots remained stranded: %+v", got)
	}
	<-running.saveJobs // 模拟已失败任务由 worker 取走。

	ctx, cancel := context.WithTimeout(context.Background(), waitDeadline)
	defer cancel()
	secondFlush := make(chan error, 1)
	go func() { secondFlush <- running.Flush(ctx) }()
	var saved []core.ChunkKey
	for {
		select {
		case err := <-secondFlush:
			...
			return
		case job := <-running.saveJobs:
			if job.Kind == saveKindChunks {
				saved = append(saved, snapshotKeys(job.Snapshots)...)
				committed := make(map[core.ChunkKey]uint64, len(job.Snapshots))
				for _, snapshot := range job.Snapshots {
					committed[snapshot.Key] = snapshot.Revision
				}
				running.saveCompletions <- saveCompletion{
					Job: job, Result: storage.SaveResult{Committed: committed},
				}
			} else {
				running.saveCompletions <- saveCompletion{Job: job}
			}
		case <-ctx.Done():
			t.Fatalf("retry flush timed out: %v", ctx.Err())
		}
	}
}
...
func TestDueRetryQueueFullKeepsAttemptAndSnapshot(t *testing.T) {
	...
	running.dispatchDueRetriesLocked(5)
...
func TestDueRetryIsQueuedBeforeFreshAutosaveSnapshot(t *testing.T) {
	...
	running.schedulePersistenceLocked(engine.TickCount())
...
func TestSameRegionFreshFailureDoesNotInheritInflightRetryAttempts(t *testing.T) {
	...
	running.retainFailedSaveLocked(saveJob{
		Region: region, Snapshots: snapshots[:1], Attempt: 9, Retry: true,
	}, snapshots[:1], errors.New("A attempt 9 failed"))
	running.dispatchDueRetriesLocked(saturatingAddUint64(engine.TickCount(), 256))
	...
	_ = running.drainSaveCompletionsLocked()
	running.dispatchDueRetriesLocked(engine.TickCount() + 1)
...
func TestOldestDueRetryPreventsFixedRegionStarvation(t *testing.T) {
	...
	for _, key := range keys {
		...
		running.retainFailedSaveLocked(
	...
	running.dispatchDueRetriesLocked(1)
	...
	_ = running.drainSaveCompletionsLocked()
	running.dispatchDueRetriesLocked(2)
