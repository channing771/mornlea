...
func hostileObserveFixture(id uint64, x float32) sim.HostileMob {
	return sim.HostileMob{
		...
		BurnCooldown:    20,
		HasTarget:       true,
		PlayerID:        playerIdentity(7).PlayerID,
		NextRepathTicks: 30,
		DistantTicks:    7,
	}
}
...
func hostilePersistenceTestConfig() Config {
...
func waitForHostileFlushWait(t *testing.T, p *hostilePersistence) {
	t.Helper()
...
func pollHostilePersistenceUntil(t *testing.T, p *hostilePersistence, tick uint64, done func() bool) {
	t.Helper()
...
func pollHostilePersistenceError(t *testing.T, p *hostilePersistence, tick uint64) error {
	t.Helper()
	deadline := time.Now().Add(waitDeadline)
	for {
		if err := p.Poll(tick); err != nil {
