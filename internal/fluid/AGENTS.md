...
`internal/fluid` 提供与权威模拟解耦的纯流体推进：有界待更新队列（`Queue`）、
...
- `Replaceable` 判定表保留在 `rules.go` 作冻结判定面：生产路径零调用（现存
  命中仅为 `internal/sim/runtime/fluid.go` 测试镜像与测试文件），供 oracle、
  性质测试与 fuzz 不变量引用。
- 差分强制点：`TestFluidEvalBatchMatchesOracle`（eval 逐位差分）与 realm 侧
  ...
  `internal/sim/realm/environment_oracle_test.go`）。
...
- 定点测试：`go test ./internal/fluid -race -count=1`。
- 重扫差分门禁：`go test ./internal/sim/realm -run TestRescanDifferential -race -count=1`。
