package network

import (
	"errors"
	"fmt"

	"github.com/channing771/mornlea/internal/core"
	"github.com/channing771/mornlea/internal/network/protocol"
)

// 夜行者域消息的 wire 编解码函数：与 companion_wire.go 同理归编解码簇（本
// 包），由 `codec_server.go` 分发调度；消息 DTO 与 wire 上限常量定义在
// `internal/network/protocol`，编解码原语是本包 unexported 类型，因此以包内
// unexported 形式留守，待编解码子包落地时随簇一并迁入并恢复包内直呼。

func encodeHostileSpawn(e *byteEncoder, spawn protocol.HostileSpawn) {
	e.u64(spawn.ServerTick)
	e.u8(uint8(len(spawn.Spawns)))
	for _, record := range spawn.Spawns {
		e.u64(record.ID)
		e.i32(int32(record.Dimension))
		for _, value := range record.Position {
			e.f32(value)
		}
		e.f32(record.Yaw)
		e.u8(record.Health)
	}
}

func encodeHostileState(e *byteEncoder, state protocol.HostileState) {
	e.u64(state.ServerTick)
	e.u8(uint8(len(state.States)))
	for _, record := range state.States {
		e.u64(record.ID)
		for _, value := range record.Position {
			e.f32(value)
		}
		for _, value := range record.Velocity {
			e.f32(value)
		}
		e.f32(record.Yaw)
		e.u8(record.Health)
	}
}

func encodeHostileDespawn(e *byteEncoder, despawn protocol.HostileDespawn) {
	e.u64(despawn.ServerTick)
	e.u8(uint8(len(despawn.IDs)))
	for _, id := range despawn.IDs {
		e.u64(id)
	}
}

func decodeHostileSpawn(d *byteDecoder) (protocol.ServerPacket, error) {
	var spawn protocol.HostileSpawn
	var err error
	if spawn.ServerTick, err = d.u64(); err != nil {
		return nil, err
	}
	count, err := d.u8()
	if err != nil {
		return nil, err
	}
	if count < 1 || int(count) > protocol.MaxHostileRecords {
		return nil, fmt.Errorf("network: hostile spawn count is outside 1..%d", protocol.MaxHostileRecords)
	}
	if len(d.data)-d.offset != int(count)*protocol.HostileSpawnWireBytes {
		return nil, errors.New("network: hostile spawn length does not match count")
	}
	spawn.Spawns = make([]protocol.HostileSpawnRecord, int(count))
	for index := range spawn.Spawns {
		record := &spawn.Spawns[index]
		if record.ID, err = d.u64(); err != nil {
			return nil, err
		}
		var dimension int32
		if dimension, err = d.i32(); err != nil {
			return nil, err
		}
		record.Dimension = core.DimensionID(dimension)
		for component := range record.Position {
			if record.Position[component], err = d.f32(); err != nil {
				return nil, err
			}
		}
		if record.Yaw, err = d.f32(); err != nil {
			return nil, err
		}
		if record.Health, err = d.u8(); err != nil {
			return nil, err
		}
	}
	return spawn, nil
}

func decodeHostileState(d *byteDecoder) (protocol.ServerPacket, error) {
	var state protocol.HostileState
	var err error
	if state.ServerTick, err = d.u64(); err != nil {
		return nil, err
	}
	count, err := d.u8()
	if err != nil {
		return nil, err
	}
	if count < 1 || int(count) > protocol.MaxHostileRecords {
		return nil, fmt.Errorf("network: hostile state count is outside 1..%d", protocol.MaxHostileRecords)
	}
	if len(d.data)-d.offset != int(count)*protocol.HostileStateWireBytes {
		return nil, errors.New("network: hostile state length does not match count")
	}
	state.States = make([]protocol.HostileStateRecord, int(count))
	for index := range state.States {
		record := &state.States[index]
		if record.ID, err = d.u64(); err != nil {
			return nil, err
		}
		for component := range record.Position {
			if record.Position[component], err = d.f32(); err != nil {
				return nil, err
			}
		}
		for component := range record.Velocity {
			if record.Velocity[component], err = d.f32(); err != nil {
				return nil, err
			}
		}
		if record.Yaw, err = d.f32(); err != nil {
			return nil, err
		}
		if record.Health, err = d.u8(); err != nil {
			return nil, err
		}
	}
	return state, nil
}

func decodeHostileDespawn(d *byteDecoder) (protocol.ServerPacket, error) {
	var despawn protocol.HostileDespawn
	var err error
	if despawn.ServerTick, err = d.u64(); err != nil {
		return nil, err
	}
	count, err := d.u8()
	if err != nil {
		return nil, err
	}
	if count < 1 || int(count) > protocol.MaxHostileRecords {
		return nil, fmt.Errorf("network: hostile despawn count is outside 1..%d", protocol.MaxHostileRecords)
	}
	if len(d.data)-d.offset != int(count)*protocol.HostileDespawnWireBytes {
		return nil, errors.New("network: hostile despawn length does not match count")
	}
	despawn.IDs = make([]uint64, int(count))
	for index := range despawn.IDs {
		if despawn.IDs[index], err = d.u64(); err != nil {
			return nil, err
		}
	}
	return despawn, nil
}
