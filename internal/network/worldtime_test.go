...
func TestProtocolV31PlayerStateCarriesDayPhaseOffset(t *testing.T) {
	for _, offset := range []uint16{0, 1, 12399, 23999} {
		state := PlayerState{Dimension: core.Overworld, DayPhaseOffset: offset, WorldTimeTicks: 0x0102030405060708}
		if err := state.Validate(); err != nil {
		...
		if round.(PlayerState).DayPhaseOffset != offset {
			t.Fatalf("往返偏移 = %d，想要 %d", round.(PlayerState).DayPhaseOffset, offset)
		}
...
// 不得输出部分 PlayerState。wire 侧从严拒绝是刻意的语义分界——存储侧（世界
// metadata 装配）对历史存档里的越界旧值宽容归一，wire 侧只传播权威单值、没有
// 兼容包袱。
func TestProtocolV31PlayerStateRejectsOutOfRangeDayPhaseOffset(t *testing.T) {
	invalid := PlayerState{Dimension: core.Overworld, DayPhaseOffset: core.DayLengthTicks}
	if err := invalid.Validate(); err == nil {
	...
	valid := PlayerState{Dimension: core.Overworld, DayPhaseOffset: 0x0101, WorldTimeTicks: 24000}
	id, payload, err := encodeServerControlPayload(StatePlay, valid)
	if err != nil {
		t.Fatal(err)
	}
...
// PlayerState wire 载荷尾部各字段的字节宽度（v31 起）：
//
...
// playerStateDayPhaseOffsetOffset 返回显示相位偏移低字节在 `PlayerState` 载荷中的下标。
