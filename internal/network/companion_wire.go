...
// （本包），由 `codec_server.go` 分发调度。消息 DTO 与 wire 上限常量定义在
// `internal/network/protocol`；编解码原语 `byteEncoder`/`byteDecoder` 是本包
// unexported 类型，因此这组函数以包内 unexported 形式留守，待编解码子包
// 落地时随簇一并迁入并恢复包内直呼。
