...
func TestProtocolV24PlayerStateCarriesHunger(t *testing.T) {
	for _, hunger := range []uint8{0, 1, 12, core.MaxHunger} {
		state := protocol.PlayerState{Dimension: core.Overworld, Hunger: hunger, WorldTimeTicks: 24000}
		...
		if got := round.(protocol.PlayerState).Hunger; got != hunger {
...
func TestProtocolV24PlayerStateRejectsOutOfRangeHunger(t *testing.T) {
	invalid := protocol.PlayerState{Dimension: core.Overworld, Hunger: core.MaxHunger + 1}
	...
	valid := protocol.PlayerState{Dimension: core.Overworld, Hunger: core.MaxHunger, WorldTimeTicks: 24000}
...
func TestProtocolV24PlayerInputCarriesEating(t *testing.T) {
	for _, tc := range []struct{ mining, eating bool }{
		...
		input := protocol.PlayerInput{Sequence: 7, Mining: tc.mining, Eating: tc.eating}
		id, payload, err := encodeClientPacketPayload(protocol.StatePlay, input)
		if err != nil {
			t.Fatal(err)
		}
		...
		got := round.(protocol.PlayerInput)
		...
		// 索引会静默改指到新字段——protocol.PlayerState 的 healthOffset 就这么错过一次。
		if len(payload) != playerInputPayloadBytes {
			t.Fatalf("protocol.PlayerInput 载荷 %d 字节，想要 %d——尾部布局变了，"+
...
// protocol.PlayerInput 的 wire 布局（v28 起，固定长度）：
//
