...
func TestProtocolV1SmallPacketGolden(t *testing.T) {
	...
	clients := []struct {
		...
		{"hello", protocol.StateHandshake, protocol.ClientHello{ProtocolVersion: 31}, 0, "1f"},
	...
	servers := []struct {
		...
		{"server hello", protocol.StateHandshake, protocol.ServerHello{ProtocolVersion: 31}, 0, "1f"},
		{"handshake reject", protocol.StateHandshake, protocol.HandshakeReject{ServerProtocolVersion: 31, Code: protocol.HandshakeVersionMismatch, Message: "no"}, 1, "1f01026e6f"},
