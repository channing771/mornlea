...
func TestProtocolV26PlaceBlockSucceededWire(t *testing.T) {
	packet := PlaceBlockSucceeded{Sequence: 0x1122334455667788}
	...
	if err != nil || packetID != 20 || hex.EncodeToString(payload) != "8877665544332211" {
		t.Fatalf("PlaceBlockSucceeded id=%d payload=%x err=%v", packetID, payload, err)
	}
	decoded, err := decodeServerControlPayload(StatePlay, packetID, payload)
	if err != nil || !reflect.DeepEqual(decoded, packet) {
		t.Fatalf("PlaceBlockSucceeded round=%#v err=%v", decoded, err)
	}
	for length := 0; length < len(payload); length++ {
		if _, err := decodeServerControlPayload(StatePlay, packetID, payload[:length]); err == nil {
			t.Fatalf("PlaceBlockSucceeded 截断到 %d bytes 仍被接受", length)
		}
	}
	if _, err := decodeServerControlPayload(StatePlay, packetID, append(payload, 0)); err == nil {
		t.Fatal("PlaceBlockSucceeded 带尾随字节仍被接受")
	}
	if _, _, err := encodeServerControlPayload(StateLogin, packet); err == nil {
		t.Fatal("PlaceBlockSucceeded 在 Login state 仍可编码")
	}
}
...
func TestProtocolV26PlaceBlockSucceededRegistryBoundary(t *testing.T) {
	...
	if _, ok := serverPacketForID(StatePlay, 22); ok {
		t.Fatal("Play S→C ID 22 必须保持未分配")
	}
	if _, err := decodeServerControlPayload(StatePlay, 22, nil); !errors.Is(err, errUnknownPacketID) {
		t.Fatalf("Play S→C ID 22 解码错误=%v，想要 %v", err, errUnknownPacketID)
	}
}
