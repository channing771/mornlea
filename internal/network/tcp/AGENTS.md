### 包边界 (`internal/network/tcp/tcp.go`、`internal/network/tcp/stream.go`)

`ListenTCP` 与 `DialTCP` 只把 TCP listener、连接、stream 和 socket 生命周期接到父包 `internal/network` 的接口上。除标准库外，本包只能单向依赖父包；packet、schema、codec 与 login 状态机均由父包持有，TCP 层不得加入 gameplay 决策或另建登录路径。
...
### 所有权与取消 (`internal/network/tcp/stream.go`、`internal/network/tcp/tcp_test.go`)
...
### Socket 与关闭语义 (`internal/network/tcp/tcp.go`、`internal/network/tcp/stream.go`)
...
### Framing 与回归边界 (`internal/network/tcp/transport_consistency_test.go`、`internal/network/tcp/tcp_test.go`、`internal/network/tcp/login_tcp_regression_test.go`)
...
### 定点验证 (`internal/network/tcp/`)

- 修改本子包：`go test ./internal/network/tcp -race -count=1`
- 修改父包共享 packet、codec、frame、login 或 stream 契约：`go test ./internal/network/... -race -count=1`
