	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := Encode(ChunkSave{
				...
				t.Fatalf("编码非法熔炉槽 error = %v，想要 storagedef.ErrCorrupt", err)
			}
		})
	}
}
...
func TestChunkCodecRejectsWrongFurnaceSlotCount(t *testing.T) {
	...
	if _, err := decodeLogicalChunk(key, 19, currentChunkSchema, truncated); !errors.Is(err, storagedef.ErrCorrupt) {
		t.Fatalf("截断熔炉负载 error = %v，想要 storagedef.ErrCorrupt", err)
	}
...
func TestChunkCodecRejectsFutureSchema(t *testing.T) {
	key := core.ChunkKey{Dimension: core.Overworld, Pos: core.ChunkPos{X: -3, Z: 7}}
	if _, _, err := migrateChunk(currentChunkSchema+1, chunkDTO{}); !errors.Is(err, storagedef.ErrFutureVersion) {
		t.Fatalf("未来 schema error = %v，想要 storagedef.ErrFutureVersion", err)
	}
	_ = key
}
