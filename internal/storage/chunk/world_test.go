...
func TestDoorRoundTrip(t *testing.T) {
	...
	encoded, err := Encode(ChunkSave{Key: key, Revision: 42, Chunk: chunk})
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
	got, err := Decode(key, 42, encoded)
	if err != nil {
	...
	// 确定性：二次编码字节一致
	second, err := Encode(ChunkSave{Key: key, Revision: 42, Chunk: chunk})
	if err != nil {
...
func TestDoorBlockIDsRoundTripExhaustive(t *testing.T) {
	...
	for id := core.DoorLowerSouthClosed; id <= core.DoorUpper; id++ {
		...
		encoded, err := Encode(ChunkSave{Key: key, Revision: 1, Chunk: chunk})
		if err != nil {
			t.Fatalf("门 %d 编码失败: %v", id, err)
		}
		got, err := Decode(key, 1, encoded)
		if err != nil {
