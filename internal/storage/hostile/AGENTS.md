...
- `StoredHostileMob`/`StoredHostileMobs`/`HostileMobsSave` 在本包内自包含
  定义：权威侧的夜行者身体类型属于 `internal/sim`，存储子树不得依赖
...
- 定点测试：`go test ./internal/storage/hostile -race -count=1`（纯
  codec 域，秒级，不编译执行其他域的测试）。
- 根包编排：`go test ./internal/storage -run 'HostileStore|DiskHostile' -race -count=1`。
