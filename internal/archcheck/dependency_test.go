...
// 语言层面不可被子包导入，但允许边表仍显式留空该方向，让契约可读而非依赖
// 编译器兜底。
...
// app/benchmark/capture 三个子包，capture、benchmark 与 devcapture 必须经
// app 访问宿主状态（devcapture 经此实现 `CaptureCoordinator` 并消费 app 状态
// 访问器）。只查「禁止边」守不住悄悄改走旁路的装配（例如 main 绕过 capture
// 的公开入口直取其内部符号），必需边消失同样是方向契约的漂移。
	...
	for pkg, packageImports := range edges {
		...
		for _, dependency := range packageImports {
			...
			violations = append(violations, fmt.Sprintf(
				"客户端命令包 %s 不允许依赖 %s：方向必须是 main → app/capture/benchmark、capture/benchmark/devcapture → app，且 capture 与 benchmark 互不依赖",
				pkg, dependency))
		}
	}
