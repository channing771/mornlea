...
// contracts、shared 与 server 是独立 Go 模块（contracts 的 go:embed 必须与
// JSON 契约同目录；shared 承载双侧共享的领域包；server 承载服务端域）；经
// 本地 replace 引用，保证 GOWORK=off 的单模块世界仍可构建。
...
replace github.com/channing771/mornlea/packages/server => ./packages/server
