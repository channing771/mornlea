//! The checked passive-mob identity the three passive families share.
//!
//! `PassiveId` is the domain's nonzero `u64` newtype, re-exported here beside
...
//! with its `network: passive spawn ID is zero`, `network: passive state ID is
//! zero` and `network: passive despawn ID is zero` messages. No other
...
//!
//! The shim mirrors `src/hostile_id.rs` rather than generalizing it: the two
//! identities are distinct domain newtypes over the same bits, so a shared
//! generic module would erase the type distinction the domain keeps.

use crate::bytes::{ByteDecoder, SliceWriter};
use crate::error::ProtocolError;
use mornlea_domain::DomainError;
pub use mornlea_domain::PassiveId;

/// Fixed encoded length of one passive identity: a little-endian `u64`.
pub const PASSIVE_ID_WIRE_BYTES: usize = 8;
...
/// answering a zero passive mob as a range violation.
pub(crate) fn read(decoder: &mut ByteDecoder<'_>) -> Result<PassiveId, ProtocolError> {
    let id = decoder.u64()?;
    PassiveId::try_new(id).map_err(|error| match error {
...
pub(crate) fn write_into(id: PassiveId, writer: &mut SliceWriter<'_>) {
    writer.u64(id.get());
}
