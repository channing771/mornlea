...
function fail(): never {
  throw new BridgeProtocolError("非法游戏视图语义");
}
...
function size(v: unknown): 2 | 3 {
  if (v !== 2 && v !== 3)
    return fail();
  return v;
}
function slots(v: unknown,
  n: number): readonly HudSlot[] {
  if (!Array.isArray(v) || v.length !== n)
    return fail();
  return v.map((s, i) => parseHudSlot(s, `game.slots[${i}]`));
}
function slotRef(raw: unknown): GameSlotRef {
  const r = object(raw, ["area", "index"]);
  if (typeof r.area !== "string" || !Object.hasOwn(limits, r.area))
    return fail();
  const area = r.area as SlotArea;
  return {
    area,
    index: integer(r.index, 0, limits[area])
  };
}
...
export function parseGame(raw: unknown): GameState {
  ...
  if (!Array.isArray(r.recipes) || r.recipes.length !== 10)
    return fail();
  const recipes = r.recipes.map(raw => {
    const recipe = object(raw, ["name", "size", "slots", "output"]);
    if (typeof recipe.name !== "string" || [...recipe.name].length > 64)
      return fail();
    return {
      name: recipe.name,
      size: size(recipe.size),
      slots: slots(recipe.slots, 9),
      output: parseHudSlot(recipe.output, "game.recipe.output")
    };
  });
  return {
    token: integer(r.token, 1, Number.MAX_SAFE_INTEGER),
    kind: r.kind as GameKind,
    cursorFree: bool(r.cursorFree),
    confirmed: bool(r.confirmed),
    inventory: slots(r.inventory, 36),
    grid: slots(r.grid, 9),
    gridSize: size(r.gridSize),
    output: parseHudSlot(r.output, "game.output"),
    chest: slots(r.chest, 27),
    furnace: slots(r.furnace, 3),
    progress: ratio(r.progress),
    burn: ratio(r.burn), recipes,
