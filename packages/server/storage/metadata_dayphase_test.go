...
// TestMetadataV4GoldenBytes 冻结当前版本的字节布局：v3 的全部既有字段原位不动
// （纯尾部追加），偏移 u64 紧随世界时间之后，天气种类 1 字节与剩余时长 u32
// 再其后，CRC32C 收尾。布局一旦漂移，这里逐字节比对会先红。
func TestMetadataV4GoldenBytes(t *testing.T) {
	metadata := Metadata{
...
// TestMetadataV2MigratesToV4WithZeroOffsetAndClearWeather 覆盖 Scenario
// 「v2 世界迁移偏移为零」与「v3 世界迁移默认晴天」的旧档一半：种子、出生信息与
// 世界时间必须原值保留，偏移迁移为 0，天气迁移为晴天、剩余时长为零，
// 读入即规范为当前版本。
func TestMetadataV2MigratesToV4WithZeroOffsetAndClearWeather(t *testing.T) {
	legacy := Metadata{
		FormatVersion:  legacyMetadataV2Version,
		Seed:           -42,
		SpawnDimension: core.DimensionID(-3),
...
// TestMetadataV1MigratesToV4WithZeroTimeOffsetAndClearWeather 锁定 v1 的三步迁移链：
// 世界时间与偏移双双为零，天气为晴天、剩余时长为零，种子与出生信息原值保留。
func TestMetadataV1MigratesToV4WithZeroTimeOffsetAndClearWeather(t *testing.T) {
	legacy := Metadata{
...
// TestMetadataVersionsRejectMalformedBytes 是四个版本的共同损坏拒绝矩阵：
// 未来版本拒绝、零版本拒绝、每个版本截断拒绝、声明其他版本载荷长度拒绝、
// CRC 损坏拒绝。矩阵覆盖全部四个版本的两两长度混淆，任何一段长度校验被删
// 都会让这里变红。
func TestMetadataVersionsRejectMalformedBytes(t *testing.T) {
	...
	v4 := mustEncodeMetadataForTest(t, current)
