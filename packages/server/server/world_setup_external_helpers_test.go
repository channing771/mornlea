...
func newMemoryAttachedWorldWithHotbar(
	...
	running := server.NewWorld(config, generator, storage.NewMemory(storage.Metadata{FormatVersion: 4, Seed: config.Seed, SpawnDimension: config.SpawnDimension, SpawnAnchor: config.SpawnAnchor}))
