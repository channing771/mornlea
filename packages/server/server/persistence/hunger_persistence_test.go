...
func flushHungerPersistence(t *testing.T, persistence *Players) {
	t.Helper()
...
func TestHungerSurvivesDiskRestart(t *testing.T) {
	...
	reopened := NewPlayers(restarted, playerPersistenceTestConfig())
...
//
// 输入是 packages/server/storage/player/testdata/player-v6.bin 的原始字节，不是当前编码器
...
func TestHungerPublishedOverWireOnLogin(t *testing.T) {
	// 在 persistence 子包中，仅验证饥饿状态经 Players 往返持久化正确，
	// wire 发布由上层 server 集成测试覆盖。
	root := t.TempDir()
	id := playerID(0x73)
	const name = "Wired"
	seedHungerPlayer(t, root, id, name, hungerTestSnapshot(savedHunger, savedSaturationMilli, savedExhaustionMilli))
	store := openHungerDiskStore(t, root, false)
	persistence := NewPlayers(store, playerPersistenceTestConfig())
	defer persistence.Close()
	restore, err := persistence.Prepare(context.Background(), id, name, testMetadata())
	if err != nil {
		t.Fatalf("Prepare: %v", err)
	}
	assertRestoreHunger(t, "Players", restore, savedHunger, savedSaturationMilli, savedExhaustionMilli)
}
