...
func TestServerProtocolV26IsCurrent(t *testing.T) {
	...
	// v41 潜行位、v42 护甲点数与装备互换命令、v43 投射物三消息、
	// v44 added the two stack-splitting commands, and v45 added whole-stack
	// dropping. The historical test name remains; this only pins the dedicated
	// server to the current protocol.
	if network.ProtocolVersion != 45 {
		t.Fatalf("专用服务端协议版本 = %d，想要 45", network.ProtocolVersion)
	}
}
...
func TestRunMigrateMaterialsCompletesAndRerunsWithSameArguments(t *testing.T) {
	...
	// Migration only changes world metadata; the protocol contract remains v45.
	if network.ProtocolVersion != 45 {
