...
func TestRuntimeComposesRealmAndEntity(t *testing.T) {
	...
	t.Run("runtime 不保留实体镜像字段", func(t *testing.T) {
		...
		for index := 0; index < engineType.NumField(); index++ {
			...
			switch {
			case elem.PkgPath() == "github.com/channing771/mornlea/packages/server/sim/realm" && elem.Name() == "State":
				realmOwners++
			case elem.PkgPath() == "github.com/channing771/mornlea/packages/server/sim/entity" && elem.Name() == "State":
				entityOwners++
			}
		}
