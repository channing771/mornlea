...
// 双格写入，床头写入失败回滚床尾（门先例）。
func (engine *engineContext) tryPlaceBed(dimensionID core.DimensionID, foot core.BlockPos, dir int, pending *pendingChunkChanges) (RejectReason, bool) {
	if dir < 0 || dir > 3 {
...
// 返回拒绝原因与是否被拒绝。
func (engine *engineContext) clearBedPair(
...
// 返回拒绝原因与是否被拒绝。
func (engine *engineContext) removeBedWithDrop(
	dimensionID core.DimensionID,
...
func (engine *engineContext) sweepUnsupportedBeds(
	pending *pendingChunkChanges,
) {
...
// 复核的权威状态。
func (engine *engineContext) invalidateBedSupportedBy(
	dimensionID core.DimensionID,
	position core.BlockPos,
	pending *pendingChunkChanges,
) {
