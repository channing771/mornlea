...
func (state *State) advanceGrassSpread(
	...
	if !sampler.GrassSpreadRoll(state.environment.seed, tick, dimensionID, position) {
		return false
	}
	...
	for _, offset := range grassSpreadNeighborOffsets {
		neighborBlock, neighborReady := dimension.BlockAt(core.BlockPos{
			X: position.X + offset.X,
			Y: position.Y,
			Z: position.Z + offset.Z,
		})
		state.environment.cropBlockReads++
		if neighborReady && neighborBlock == core.GrassID {
	...
	mutation.Record(dimensionID, position, core.GrassID)
	return true
}
