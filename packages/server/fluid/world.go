...
// 待更新队列编排。单格求值经 packages/shared/nativeabi 批量送入 Rust engine kernel
// （见 eval_native.go）；待办存储与按全序取批由 packages/server/updates 的统一
// 调度器承载（见 queue.go），除此之外本包只依赖 packages/shared/core，不依赖
// packages/shared/world 与 internal/sim/network/render/storage——流动延迟与
// 每 tick 预算这两个 tunable 归 internal/sim 所有，一律由调用方在调用时以参数
// 传入，本包不定义、不读取任何隐藏默认值。
//
