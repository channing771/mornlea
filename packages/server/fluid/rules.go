...
// 树苗由 oak-sapling-regrowth 追加，见 design.md D1/D5/D8）：
//
//	目标格                        可替换？
//	空气                          是
//	流体等级更大（更弱）的流动水    是
//	植物（作物任意生长阶段、短草、树苗）  是——水淹即冲毁/清除；冲毁后的掉落结算由
//	                                权威写入侧（sim）在流体写入汇聚点完成，本包不
//	                                感知物品。作物按采掘同表掉落，树苗掉落 1 个自身
//	                                且与作物同受容量原子拒绝约束，短草零掉落且不受
//	                                容量限制
//	源方块                        否——「源不可被流动方块替换」
...
func Replaceable(target core.BlockID, newLevel uint8) bool {
	...
	if core.IsPlant(target) {
		// 植物（作物、短草与树苗）对流动水可替换。放行点必须在本函数而不能在
		...
		// 只回答「能不能写」：作物与树苗被冲毁后的掉落结算是权威写入侧（sim）
		// 的职责，短草被覆盖则零掉落、不做容量预演——三者的区分同样留在 sim
		// 写入侧，fluid 包不感知物品。
		return true
	}
