...
// updateDomainEventWorldCorpus rewrites the frozen event world corpus from the
...
var updateDomainEventWorldCorpus = flag.Bool(
	"update-domain-event-world-corpus",
	false,
	"rewrite testdata/runtime-migration/cases/domain/event_world from the executed protocol world observation DTOs",
)
...
func domainEventWorldSyncCorpus(t *testing.T, records []domainEventWorldRecord) {
	...
	if *updateDomainEventWorldCorpus {
	...
	for relative, data := range want {
		...
		if err != nil {
			t.Fatalf("read frozen corpus case %s: %v (rerun with -update-domain-event-world-corpus after reviewing the change)", relative, err)
		}
		if !bytes.Equal(committed, data) {
			t.Errorf("frozen corpus case %s drifted from the executed protocol DTO (rerun with -update-domain-event-world-corpus after reviewing the change)", relative)
		}
	}
