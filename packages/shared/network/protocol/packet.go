...
// ProtocolVersion is the only supported version. v45 appends Play C-to-S ID 21,
// `DropStack`, as a fixed 28-byte command matching `MoveStackPartial` addressing.
// Views outside {0,1,2}, mismatched container references, and slot overflow past
// 35/44/62|38 reject the packet. The server derives position and full-stack count
// from authoritative state; the wire carries neither and clients do not predict.
// Empty slots and invalid references use existing simulation rejection semantics;
// v44 appends to the Play C-to-S tail:
...
//
// v45 is append-only: it adds Play C-to-S ID 21 `DropStack` without changing
// existing wire shapes, limits, or `RejectReason` values. Older handshakes retain
...
// 不新增 C→S 消息、不改动既有 packet 的 wire 形状与全部长度上限、不新增
// `RejectReason`。v32 是纯追加：只在 Play S→C 尾部新增 ID 25 的 `CombatHit`，不新增 C→S 消息、
...
const ProtocolVersion uint32 = 45
