...
func TestProtocolV1SmallPacketGolden(t *testing.T) {
	...
	clients := []struct {
		...
		{"hello", protocol.StateHandshake, protocol.ClientHello{ProtocolVersion: 33}, 0, "21"},
	...
	servers := []struct {
		...
		{"server hello", protocol.StateHandshake, protocol.ServerHello{ProtocolVersion: 33}, 0, "21"},
		{"handshake reject", protocol.StateHandshake, protocol.HandshakeReject{ServerProtocolVersion: 33, Code: protocol.HandshakeVersionMismatch, Message: "no"}, 1, "2101026e6f"},
