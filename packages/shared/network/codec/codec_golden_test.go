...
func TestProtocolV1SmallPacketGolden(t *testing.T) {
	...
	clients := []struct {
		...
		{"hello", protocol.StateHandshake, protocol.ClientHello{ProtocolVersion: 45}, 0, "2d"},
	...
	servers := []struct {
		...
		{"server hello", protocol.StateHandshake, protocol.ServerHello{ProtocolVersion: 45}, 0, "2d"},
