...
func TestProtocolV11InventoryCarriesWornToolDurability(t *testing.T) {
	...
	if packetID != 10 || len(payload) != 1+core.InventorySlots*5 {
		t.Fatalf("protocol.InventoryState id=%d payload=%d，想要 id=10 payload=181", packetID, len(payload))
	}
...
func TestProtocolV15InventoryCarriesLightBlockItem(t *testing.T) {
	...
	got, ok := decoded.(protocol.InventoryState)
	if !ok {
		t.Fatalf("发光块物品背包解码类型=%T，想要 protocol.InventoryState", decoded)
	}
...
func TestGridCraftingMoveValueDomain(t *testing.T) {
	...
	for _, command := range valid {
		...
		id, payload, err := encodeClientPacketPayload(protocol.StatePlay, command)
		if err != nil || id != 7 {
			t.Fatalf("编码 %+v = id %d, err %v，想要 id=7", command, id, err)
		}
		round, err := decodeClientPacketPayload(protocol.StatePlay, id, payload)
		if err != nil || round != (protocol.ClientPacket)(command) {
			t.Fatalf("往返 %+v = %+v, %v", command, round, err)
		}
	}
	invalid := []protocol.MoveCraftingStack{
	...
	for _, wire := range [][]byte{
		...
		if packet, err := decodeClientPacketPayload(protocol.StatePlay, 7, wire); err == nil {
...
func TestTakeCraftingOutputRequiresNonZeroSequence(t *testing.T) {
	command := protocol.TakeCraftingOutput{Sequence: 0x1122334455667788}
	if err := command.Validate(); err != nil {
	...
	if round, err := decodeClientPacketPayload(protocol.StatePlay, id, payload); err != nil || round != (protocol.ClientPacket)(command) {
		t.Fatalf("取出命令往返 = %+v, %v", round, err)
	}
	if err := (protocol.TakeCraftingOutput{}).Validate(); err == nil {
		t.Fatal("零序号取出命令通过了 Validate")
	}
	if _, _, err := encodeClientPacketPayload(protocol.StatePlay, protocol.TakeCraftingOutput{}); err == nil {
		t.Fatal("零序号取出命令被编码接受")
	}
	if packet, err := decodeClientPacketPayload(protocol.StatePlay, 15, make([]byte, 8)); err == nil {
...
func goldenCraftingState() protocol.CraftingState {
...
func TestCraftingStateWireAndValueDomain(t *testing.T) {
	...
	if id != 21 || len(payload) != 1+core.CraftingGridSlots*5+5 {
		t.Fatalf("protocol.CraftingState id=%d payload=%d，想要 id=21 且固定 51 字节", id, len(payload))
	}
	if got := hex.EncodeToString(payload); got != "03"+"0100020000"+"0000000000"+"0000000000"+"0000000000"+
		...
		t.Fatalf("protocol.CraftingState wire=%s", got)
	}
	decoded, err := decodeServerControlPayload(protocol.StatePlay, id, payload)
	if err != nil {
		t.Fatalf("解码网格状态: %v", err)
	}
	if got, ok := decoded.(protocol.CraftingState); !ok || got != packet {
	...
	for length := 0; length < len(payload); length++ {
		if _, err := decodeServerControlPayload(protocol.StatePlay, id, payload[:length]); err == nil {
			t.Fatalf("protocol.CraftingState 截断到 %d bytes 仍被接受", length)
		}
	}
	if _, err := decodeServerControlPayload(protocol.StatePlay, id, append(append([]byte(nil), payload...), 0)); err == nil {
		t.Fatal("protocol.CraftingState 带尾随字节仍被接受")
	}

	// 尺寸 2 时格 4..8 必须为空：这是「网格状态私有同步且有界」的 wire 硬边界
	//（sim 侧构造上不可能产出该状态，这里是 codec 对伪造载荷的防御层）。
	personal := protocol.CraftingState{Size: 2}
	personal.Slots[0] = core.ItemStack{Item: core.ItemStone, Count: 1}
	personalID, personalPayload, err := encodeServerControlPayload(protocol.StatePlay, personal)
	if err != nil {
		t.Fatalf("编码尺寸 2 状态: %v", err)
	}
	if round, err := decodeServerControlPayload(protocol.StatePlay, personalID, personalPayload); err != nil ||
		round != (protocol.ServerPacket)(personal) {
	...
	if _, _, err := encodeServerControlPayload(protocol.StatePlay, residue); err == nil {
	...
	corruptSize[0] = 4
	if _, err := decodeServerControlPayload(protocol.StatePlay, 21, corruptSize); err == nil {
		t.Fatal("尺寸 4 的 wire 载荷被解码接受")
	}
	corruptCount := append([]byte(nil), personalPayload...)
	corruptCount[3] = core.MaxStackCount + 1
	if _, err := decodeServerControlPayload(protocol.StatePlay, 21, corruptCount); err == nil {
