...
func TestProtocolV9PlayerStateCarriesWorldTime(t *testing.T) {
	...
	// v37 起再有季节三字节、v42 起再有 1 字节护甲点数，此处夹具后五者均为
	// 零值故后缀多五个 00）。
	got := hex.EncodeToString(payload)
	const wantSuffix = "0807060504030201" + "00" + "00" + "00" + "00" + "00"
...
func TestProtocolV31PlayerStateCarriesDayPhaseOffset(t *testing.T) {
	for _, offset := range []uint16{0, 1, 12399, 23999} {
		...
		// 是偏移的低/高字节，再接 8 字节小端绝对世界时间、v36 起的 1 字节天气、
		// v37 起的季节三字节与 v42 起的 1 字节护甲点数（此处夹具后五者均为
		// 零值故是五个 00）。
		got := hex.EncodeToString(payload)
		wantSuffix := fmt.Sprintf("%02x%02x", offset&0xFF, offset>>8) + "0807060504030201" + "00" + "00" + "00" + "00" + "00"
		if len(got) < len(wantSuffix) || got[len(got)-len(wantSuffix):] != wantSuffix {
...
// protocol.PlayerState wire 载荷尾部各字段的字节宽度（v42 起）：
//
//	… | Health u8 | Oxygen u16 | Hunger u8 | SaturationZero u8 | DayPhaseOffset u16 | WorldTimeTicks u64 | WeatherKind u8 | Season u8 | SeasonProgress u8 | Temperature i8 | ArmorPoints u8
//
...
// playerStateArmorPointsOffset 返回护甲点数字节在 `protocol.PlayerState` 载荷
// 中的下标。点数是载荷最末一字节（v42 起紧跟 `Temperature` 之后），它是整条
// 偏移链的最外层：后续再向尾部追加字段时只改这里，其余偏移经它自动跟上。
func playerStateArmorPointsOffset(payloadLen int) int {
	return payloadLen - playerStateArmorPointsBytes
}

// playerStateTemperatureOffset 返回玩家位置温度字节在 `protocol.PlayerState`
// 载荷中的下标。v37 起温度紧跟 `SeasonProgress` 之后（曾是载荷最末一字节，
// v42 起其后还有 1 字节护甲点数，改经点数偏移链式求值）。
func playerStateTemperatureOffset(payloadLen int) int {
	return playerStateArmorPointsOffset(payloadLen) - playerStateTemperatureBytes
}
...
func playerStateDayPhaseOffsetOffset(payloadLen int) int {
	return playerStateWeatherOffset(payloadLen) - playerStateWorldTimeBytes - playerStateDayPhaseOffsetBytes
}
