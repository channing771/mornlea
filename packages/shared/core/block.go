...
const (
	...
	// （档位 n 的 4-bit 顶面高度原值即为 n，呈现高度 (n+1)/16 = 2/16..5/16），
...
// 厚度语义：档位 n 的 4-bit 顶面高度原值即为 n（呈现高度 (n+1)/16 =
// 2/16..5/16，spec 钉的可观察厚度）；档位推进（积雪升档、消融降档、踩踏降档）
// 就是编号 ±1，调用方必须先经本谓词确认档位有效再加减，不得对区间外的编号
// 做算术。
