...
//
// 目前有面包、马铃薯、胡萝卜、毒土豆、腐肉、生牛肉、熟牛肉七种食物：面包足以验证三层状态与
// 进食状态机的全部路径，肉类需要生物（腐肉是首例外——它由夜行者死亡掉落引入，
// 不需要任何屠宰/烹饪链；生牛肉由牛死亡掉落引入，熟牛肉是前者的熔炼产物）、熟食需要熔炉食谱；马铃薯系在更多作物中追加，毒土豆
...
func FoodValue(item ItemID) (uint8, uint16, bool) {
	...
	case ItemRottenFlesh:
		return 4, 0, true
	// 生牛肉 (3, 1.8) 与熟牛肉 (8, 12.8)：与参考实现同值，档位沿既有食物表——
	// 生牛肉与胡萝卜同饥饿、取低档饱和率（马铃薯/毒土豆档每点饥饿 600 饱和），
	// 熟牛肉高于面包 (5, 6.0)、取高档饱和率（面包/胡萝卜档每点饥饿 1200 饱和）。
	// 烹饪把 1 生牛肉变成 1 熟牛肉（见 `SmeltingOutput`），饥饿与饱和都严格提升，
	// 这正是食物链闭环的升级语义。
	case ItemRawBeef:
		return 3, 1800, true
	case ItemCookedBeef:
