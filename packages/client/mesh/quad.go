...
// PlantMaterialFirst..PlantMaterialLast 与离散的 PlantMaterialShortGrass、
// PlantMaterialSapling 共同构成植物材质层集合 `[31..54] ∪ {68, 164}`。
//
// 「一格是不是植物」以 **material 为准**（design D8）：判别不占 quad 位，而 quad
// 位已经只剩 bit 63 一个且必须留空。三处必须一起对齐：
//
//   - Go：本组常量，由 packages/client/assets 的 LayerWheat0..LayerCarrot7、
//     LayerShortGrass 与 LayerSapling 提供数值，
//     两者相等由 assets 的 TestPlantMaterialLayersMatchMeshContract 钉住
//     （assets 依赖 mesh，反向不成立，所以常量住在这里、断言写在那边）；
//   - Rust engine：`quad.rs` 的 PLANT_MATERIAL_FIRST / PLANT_MATERIAL_LAST 与
//     两个离散常量，它据此决定哪些格跳过轴向面、改出 4 条交叉斜面；跨语言
//     一致性由真的喂一次 Rust mesher 的真实植物交叉斜面测试兜底；
...
const (
	...
	// PlantMaterialSapling 是树苗的离散材质层：追加在物品与人物/雪层之后的
	// 原创内部区，与短草同形的第二个单点。
	PlantMaterialSapling uint16 = 164
)
...
func PlantMaterial(mat uint16) bool {
	return mat >= PlantMaterialFirst && mat <= PlantMaterialLast ||
		mat == PlantMaterialShortGrass || mat == PlantMaterialSapling
}
