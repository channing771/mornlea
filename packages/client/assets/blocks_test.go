...
func TestBlockTopRawSinksFarmlandOnly(t *testing.T) {
	...
	// 静默退化成子集，新方块就漏出「非短方块必须为哨兵 0」的检查。
	for id := core.AirID; id < core.BlockIDMax; id++ {
		if id == core.FarmlandDryID || id == core.FarmlandWetID || core.IsSnowLayer(id) {
			continue
		}
		if got := registry.BlockTopRaw(id); got != 0 {
			t.Fatalf("非短方块 %d 的 BlockTopRaw=%d，想要满格哨兵 0", id, got)
		}
	}
}
...
func TestPlantMaterialLayersMatchMeshContract(t *testing.T) {
	...
	if assets.LayerSapling != mesh.PlantMaterialSapling {
		t.Fatalf("LayerSapling=%d，mesh.PlantMaterialSapling=%d", assets.LayerSapling, mesh.PlantMaterialSapling)
	}
