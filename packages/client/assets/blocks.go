...
const (
	...
	// （见 packages/client/render.SectionScheduler），共用石头层等于水面被画进
	// 不透明 terrain pass。
	LayerWater
	...
	// `packages/client/mesh` 的 PlantMaterialFirst/Last 用闭区间
	// [PLANT_MATERIAL_FIRST, PLANT_MATERIAL_LAST] 识别作物，另以
	// PlantMaterialShortGrass 识别离散短草层，从而决定一格出交叉斜面而非轴向面。
	// 两侧没有共享常量也没有生成步骤，只能人手同步——数值一侧由 packages/client/mesh 的植物材质常量与
...
//
// 这条转调必须保持**无条件**：packages/client/mesh/visibility.go 的
...
// 的整批区段。守卫见 packages/client/mesh 的
...
// 把结果烘焙成 Visibility 位图（见 packages/client/mesh/registry.go），Rust 的
...
// packages/client/assets 的 TestFluidFaceVisibilityRules 与 packages/client/mesh 的
