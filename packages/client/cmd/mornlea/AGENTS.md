...
| 视觉抓帧 | `--capture`（可带 `--update-golden`、`--capture-scenes`、`--capture-gifs`） | 确定性内存世界加离屏 renderer，跑完 capture 包的固定场景表，`--capture-scenes` 按场景表保序只跑子集（未知/重复名拒绝），`--capture-gifs` 让纯比对也生成 GIF 剧本（更新基线恒生成）；不能与 `--benchmark`/`--connect` 同用 |
