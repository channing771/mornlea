...
// 同一份夹具在任何机器上产出同一组方块。
//
// 支撑面由夹具自己铺出，不依赖任何既有地形：`prepareCaptureAirNeighborhood`
// 先把区块 -1..1 的全部区段填成空气，窗口内从 `MinY` 到 `MaxY` 都是空腔，相机
// 若不对着地面就会悬空、画面下缘直接露出窗口边界。夹具因此在 y=-1 铺石、
// y=0 铺草，范围 x=-5..9、z=-14..4，完全落在窗口（区块 -1..1，即 x/z ∈
// [-16,31]）之内，画面主体就是这片手工草地，与既有近景场景（短草近景、
// 水桶池塘）同口径。
