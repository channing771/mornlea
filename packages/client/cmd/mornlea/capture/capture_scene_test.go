...
// 东/西带沿 x），与 `assets.bedBand` 的画带约定一致。静态测试和正式 runner
// 共用主手抑制入口，避免局部像素断言依赖持物构图或跳过被手遮住的床。
...
func TestBedNightScenePixelsShowMultiOrientationBedsAtNight(t *testing.T) {
	...
	samples := []struct {
		...
		{"西向床", mgl32.Vec3{1.22, bedTopY, -3.5}, mgl32.Vec3{2.22, bedTopY, -3.85}, mgl32.Vec3{2.5, bedTopY, -3.5}},
