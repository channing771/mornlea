...
func parseMainOptions(args []string) (mainOptions, error) {
	...
	motionScene := flags.String("motion-scene", "break-burst", "motion 场景：break-burst/avatar-walk/drop-scatter/drop-density/hand-mining/hand-attack/weather-cycle/held-items")
	...
	if *motionScene != "break-burst" && *motionScene != "avatar-walk" && *motionScene != "drop-scatter" && *motionScene != "drop-density" && *motionScene != "hand-mining" && *motionScene != "hand-attack" && *motionScene != "weather-cycle" && *motionScene != "held-items" {
