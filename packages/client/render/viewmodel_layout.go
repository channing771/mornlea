...
func viewmodelGripRoot(input *ViewmodelInput, phase float32) mgl32.Mat4 {
	...
	pose := viewmodelPhasePose(phase, ViewmodelTierOf(input.Selected))
	depth := float32(.95) + pose.forward
	return mgl32.Translate3D((x+pose.x)*w/h*tangent*depth, (-y+pose.y)*tangent*depth, -depth).
		...
		Mul4(mgl32.HomogRotate3DZ(50 * math.Pi / 180))
}
