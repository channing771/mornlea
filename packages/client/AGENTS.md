...
- `assets.Registry` 持有全注册物品的 16×16 RGBA 只读图标缓存；完整方块从当前
  ...
  第一人称主手固定八个部件（前臂、袖口、拳掌、拇指与四个局部分面）；空手分面贴合闭合拳掌与袖子；工具以掌、下垂拇指和折指围住柄，肤色部件全部归于同一抓握框架，袖子分面保持原框架，肤色读取当前身份头底面 atlas 后作局部暖色分面，袖口/前臂取同身份袖子中部布料色，避免整张袖纹拉伸。联合预算为 264，96 字节编码与 ABI 不变。
