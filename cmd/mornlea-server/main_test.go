...
func TestServerProtocolV26IsCurrent(t *testing.T) {
	// 字面量随 `network.ProtocolVersion` 的现行契约同步（v31 在 `PlayerState`
	// 尾部追加点亮相位偏移）；函数名保留历史编号先例，仅钉住「专用服务端跟随
	// 当前协议」。
	if network.ProtocolVersion != 31 {
		t.Fatalf("专用服务端协议版本 = %d，想要 31", network.ProtocolVersion)
	}
}
...
func TestRunMigrateMaterialsCompletesAndRerunsWithSameArguments(t *testing.T) {
	...
	// 迁移只动世界 metadata，协议契约必须保持现行值（v31）不变。
	if network.ProtocolVersion != 31 {
