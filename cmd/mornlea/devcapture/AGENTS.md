...
`cmd/mornlea/devcapture` 给运行中的交互式客户端暴露仅绑定回环地址的 HTTP
...
- BGRA8→NRGBA 转换住 `bgraToNRGBA`（`devcapture/bgra.go`），有意不 import
  `cmd/mornlea/capture` 复用其未导出助手：两侧契约来源不同（离屏 readback vs
...
- 定点测试：`go test ./cmd/mornlea/devcapture -race -count=1`（`httptest`
  全覆盖，秒级）。
