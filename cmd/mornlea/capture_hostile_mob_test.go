...
func TestCaptureHostileMobFixtureIsDeterministicAndTagFree(t *testing.T) {
	...
	if app.center != cameraChunk(app.camera.Pos) {
	...
	avatars, tags := remoteRenderPresentationsSortedInto(
		nil, make([]render.NameTag, 0, maxFrameNameTags),
		app.remotePlayers.AppendPresentations(nil),
	)
	avatars, tags = appendCompanionRenderPresentationsInto(
		avatars, tags, app.companions.AppendPresentations(nil),
	)
	avatars = appendHostileRenderPresentationsInto(avatars, presentations)
	if len(tags) != 0 {
