...
生命值降到 0 时，系统在同一权威 tick 内完成死亡结算：36 个物品栏格逐格掉进世界（放置成功才清空该格，物品不会被静默销毁）、玩家传回出生锚点、生命值回满、速度归零。死亡掉落使用 `40` 个 active tick 的拾取延迟。掉落物从死亡所在区块开始按半径 0、1、2… 环形外扩寻找空位，只写入已加载且 Ready 的区块；扫完全部已加载区块仍放不下的格会保留在背包中跟随重生。外部观察不到生命值为 0 的中间状态，发布的永远是重生后的满血状态。
...
- 采掘与方块破坏掉落经过 `10` 个 active tick 后才可拾取，主动丢弃与死亡掉落则经过 `40` 个 active tick 后才可拾取；延迟结束后，1.25 格内的玩家可以拾取。掉落物累计 6000 个 active tick 后消失，且只有玩家附近（区块半径 2）的掉落物才会推进寿命；
