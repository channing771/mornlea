...
| 按住鼠标右键 | 作为「使用」键：命中熔炉/箱子时请求打开容器，锄头命中泥土/草时翻地，种子或方块物品执行放置；手持面包且饥饿未满时持续 32 tick 完成一次权威进食 |
...
眼睛浸没时，氧气从 `300` 每 tick 减 1；归零后默认每 `20` tick 经统一伤害入口扣 1 点生命，出水立即回满。饥饿值范围为 `0..20`，饱和度与疲劳使用整数千分位推进；面包是当前唯一食物，连续使用默认 `32` tick 后原子扣一件并恢复 5 点饥饿和 6000 千分位饱和度。饥饿为 0 时默认每 `80` tick 扣 1 点生命，但停在 1 点而不致死。
...
Darwin 图形客户端在采掘、放置、进食与受伤的权威确认边界播放本地程序化提示音；拒绝、预测、重复/旧放置序号以及其他会话不会触发。音频设备不可用时无声降级，capture、benchmark 与专服不请求音频设备。
...
- 主动丢弃只支持单件原地转移：服务端在同一权威 tick 内校验玩家、选中栏位、脚底区块与每区块 32 个掉落物槽后原子扣除一件并在脚下创建掉落物，固定 `40` 个活动 tick 内不可拾取（采掘与方块破坏产生的掉落物仍为 `10`）。合并到同位置旧堆时保留其 ID 与寿命时间线，只把拾取禁止窗口延长到较长的来源延迟；所有已注册物品都可作为掉落物传播；
- 尚无整组丢弃、丢弃数量选择、投掷速度、重力与水平移动、所有者专属拾取与客户端预测；
- 生命值、氧气、饥饿/饱和/疲劳、面包进食、自然回血门控、饥饿伤害、溺水、摔落与玩家近战/PvP 均由服务端权威结算，Darwin 客户端在采掘/放置/进食/受伤的确认边界播放本地提示音；当前仍无怪物、窒息/岩浆/火焰伤害、床、自定义重生点或专门的死亡界面。
